...
import { createlocalcoder } from "@localcoder-ai/sdk"
...
`)
  ...
  const localcoder = await createlocalcoder({ port: 0 })
  ...
  try {
    ...
    const session = await localcoder.client.session.create()
    const result = await localcoder.client.session
      .prompt({
