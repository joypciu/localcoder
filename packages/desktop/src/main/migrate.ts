...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.localcoder.desktop.dev",
  beta: "ai.localcoder.desktop.beta",
  prod: "ai.localcoder.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.localcoder.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `localcoder.settings.dat` is special: it maps to the `localcoder.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"localcoder.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // localcoder.settings.dat → the electron settings store ("localcoder.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "localcoder.global.dat").
  const storeName = filename === "localcoder.settings.dat" ? "localcoder.settings" : filename
