...
process.env.LOCALCODER_DISABLE_EMBEDDED_WEB_UI = "true"

const APP_NAMES: Record<string, string> = {
  dev: "LocalCoder Dev",
  beta: "LocalCoder Beta",
  prod: "LocalCoder",
}
...
const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.localcoder.desktop.dev"
app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "LocalCoder Dev")
...
function setupApp() {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("localcoder://"))
    if (urls.length) {
  ...
  void app.whenReady().then(async () => {
    migrate()
    app.setAsDefaultProtocolClient("localcoder")
    registerRendererProtocol()
    setDockIcon()
    setupAutoUpdater()
    await initialize()
...
async function getSidecarPort() {
  const fromEnv = process.env.LOCALCODER_PORT
  if (fromEnv) {
...
function sqliteFileExists() {
  ...
  return existsSync(join(base, "localcoder", "localcoder.db"))
}
