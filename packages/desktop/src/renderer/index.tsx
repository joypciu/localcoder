...
import "./styles.css"
import { useTheme } from "@localcoder-ai/ui/theme"
  ...
  Sentry.init({
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" && !(import.meta.env.LOCALCODER_CHANNEL === "prod" && i.name === "GlobalHandlers"),
      )
    },
  })
}
...
const deepLinkEvent = "localcoder:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__LOCALCODER__ ??= {}
  const pending = window.__LOCALCODER__.deepLinks ?? []
  window.__LOCALCODER__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://localcoder.ai/favicon-96x96-v3.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("localcoder.global.dat").getItem("language")
