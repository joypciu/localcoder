...
import { type Session } from "@localcoder-ai/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("localcoder://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("localcoder://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("localcoder://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("localcoder://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("localcoder://open-project")).toBeUndefined()
    expect(parseDeepLink("localcoder://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "localcoder://open-project?directory=/a",
      "localcoder://other?directory=/b",
      "localcoder://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("localcoder://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("localcoder://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("localcoder://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("localcoder://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "localcoder://new-session?directory=/a",
      "localcoder://open-project?directory=/b",
      "localcoder://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __LOCALCODER__: {
        deepLinks: ["localcoder://open-project?directory=/a"],
      },
    } as unknown as Window & { __LOCALCODER__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["localcoder://open-project?directory=/a"])
