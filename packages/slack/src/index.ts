...
import { createlocalcoder, type ToolPart } from "@localcoder-ai/sdk"
...
console.log("🚀 Starting localcoder server...")
const localcoder = await createlocalcoder({
  port: 0,
})
console.log("✅ localcoder server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await localcoder.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new localcoder session...")
    const { client, server } = localcoder
    ...
    console.log("✅ Created localcoder session:", createResult.data.id)
  ...
  console.log("📝 Sending to localcoder:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 localcoder response:", JSON.stringify(result, null, 2))
