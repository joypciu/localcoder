...
With LocalCoder you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, LocalCoder uses our own `localcoder` theme.
...
LocalCoder comes with several built-in themes.
...
- Want LocalCoder to match their terminal's appearance
...
LocalCoder supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/localcoder/themes/*.json` or `$XDG_CONFIG_HOME/localcoder/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.localcoder/themes/*.json`
4. **Current working directory** - Defined in `./.localcoder/themes/*.json`
