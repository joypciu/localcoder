...
Создайте `.localcoder/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию localcoder или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем localcoder [config](/docs/config):
...
- Глобальный: `~/.config/localcoder/commands/`
- Для каждого проекта: `.localcoder/commands/`
...
localcoder включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
