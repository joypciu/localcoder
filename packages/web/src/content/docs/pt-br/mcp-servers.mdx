...
Você pode adicionar ferramentas externas ao localcoder usando o _Model Context Protocol_, ou MCP. O localcoder suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do localcoder](https://localcoder.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/localcoder`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O localcoder lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o localcoder irá:
...
Se o servidor requer autenticação, o localcoder solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `localcoder mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o localcoder armazenará os tokens de forma segura em `~/.local/share/localcoder/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no localcoder, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do localcoder como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o localcoder à sua conta do Sentry.
