---
title: Plugins
description: Escreva seus próprios plugins para estender o localcoder.
---

Plugins permitem que você estenda o localcoder conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do localcoder.
...
- `.localcoder/plugins/` - Plugins em nível de projeto
- `~/.config/localcoder/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/localcoder/node_modules/`.
...
1. Configuração global (`~/.config/localcoder/localcoder.json`)
2. Configuração do projeto (`localcoder.json`)
3. Diretório de plugins global (`~/.config/localcoder/plugins/`)
4. Diretório de plugins do projeto (`.localcoder/plugins/`)
...
O localcoder executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do localcoder para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o localcoder.
...
:::note
Se você estiver usando o aplicativo desktop localcoder, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o localcoder de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao localcoder:
...
O helper `tool` cria uma ferramenta personalizada que o localcoder pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o localcoder junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://localcoder.ai/docs/sdk) para detalhes.
