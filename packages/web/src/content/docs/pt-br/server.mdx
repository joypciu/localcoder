---
title: Servidor
description: Interaja com o servidor localcoder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `localcoder serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente localcoder pode usar.
...
```bash
localcoder serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `localcoder.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
localcoder serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `LOCALCODER_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `localcoder`, ou defina `LOCALCODER_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `localcoder serve` quanto ao `localcoder web`.

```bash
LOCALCODER_SERVER_PASSWORD=your-password localcoder serve
```
...
Quando você executa `localcoder`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor localcoder para interagir com o localcoder programaticamente.
:::

Essa arquitetura permite que o localcoder suporte múltiplos clientes e permite que você interaja com o localcoder programaticamente.

Você pode executar `localcoder serve` para iniciar um servidor autônomo. Se você tiver o TUI do localcoder em execução, `localcoder serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do localcoder [IDE](/docs/ide).
...
O servidor localcoder expõe as seguintes APIs.
