...
Com o localcoder, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o localcoder usa nosso próprio tema `localcoder`.
...
O localcoder vem com vários temas embutidos.
...
- Querem que o localcoder corresponda à aparência do seu terminal
...
O localcoder suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/localcoder/themes/*.json` ou `$XDG_CONFIG_HOME/localcoder/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.localcoder/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.localcoder/themes/*.json`
