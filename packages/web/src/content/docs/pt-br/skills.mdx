...
As habilidades do agente permitem que o localcoder descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O localcoder pesquisa nesses locais:

- Configuração do projeto: `.localcoder/skills/<name>/SKILL.md`
- Configuração global: `~/.config/localcoder/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o localcoder sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.localcoder/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/localcoder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.localcoder/skills/git-release/SKILL.md` assim:
...
O localcoder lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `localcoder.json`:
...
**Para agentes embutidos** (em `localcoder.json`):
