...
You can add external tools to LocalCoder using the _Model Context Protocol_, or MCP. LocalCoder supports both local and remote servers.
...
You can define MCP servers in your [LocalCoder Config](https://localcoder.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/localcoder` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
LocalCoder automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, LocalCoder will:
...
If the server requires authentication, LocalCoder will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `localcoder mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, LocalCoder will store the tokens securely in `~/.local/share/localcoder/mcp-auth.json`.
...
Your MCPs are available as tools in LocalCoder, alongside built-in tools. So you can manage them through the LocalCoder config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect LocalCoder to your Sentry account.
