...
Puoi aggiungere strumenti esterni a LocalCoder usando il _Model Context Protocol_, o MCP. LocalCoder supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [LocalCoder Config](https://localcoder.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/localcoder`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
LocalCoder gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, LocalCoder:
...
Se il server richiede autenticazione, LocalCoder ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `localcoder mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, LocalCoder memorizzera' i token in modo sicuro in `~/.local/share/localcoder/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in LocalCoder insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione LocalCoder come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare LocalCoder al tuo account Sentry.
