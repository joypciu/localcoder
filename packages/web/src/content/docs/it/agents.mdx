...
In LocalCoder esistono due tipi di agenti: agenti primari e subagenti.
...
LocalCoder include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
LocalCoder include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

LocalCoder include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `localcoder.json`:
...
- Globale: `~/.config/localcoder/agents/`
- Per progetto: `.localcoder/agents/`
...
Se non specifichi una temperature, LocalCoder usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di LocalCoder sia per la config specifica del progetto.
...
L'ID modello nella configurazione LocalCoder usa il formato `provider/model-id`. Per esempio, se stai usando [LocalCoder Zen](/docs/zen), useresti `localcoder/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `localcoder models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/localcoder).
:::
