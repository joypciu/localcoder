...
Le skill degli agenti permettono a LocalCoder di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
LocalCoder cerca in queste posizioni:

- Config di progetto: `.localcoder/skills/<name>/SKILL.md`
- Config globale: `~/.config/localcoder/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, LocalCoder risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.localcoder/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/localcoder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.localcoder/skills/git-release/SKILL.md` cosi':
...
LocalCoder elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `localcoder.json`:
...
**Per agenti integrati** (in `localcoder.json`):
