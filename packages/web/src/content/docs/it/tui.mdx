---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di LocalCoder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LocalCoder fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire LocalCoder avvia la TUI per la directory corrente.
...
Quando usi la TUI di LocalCoder, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a LocalCoder. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da LocalCoder. _Alias_: `/quit`, `/q`
...
Questo è separato da `localcoder.json`, che configura il comportamento di server/runtime.
...
Usa `LOCALCODER_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
