---
title: Plugin
description: Scrivi plugin per estendere LocalCoder.
---

I plugin ti permettono di estendere LocalCoder agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di LocalCoder.
...
- `.localcoder/plugins/` - plugin a livello progetto
- `~/.config/localcoder/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/localcoder/node_modules/`.
...
1. Config globale (`~/.config/localcoder/localcoder.json`)
2. Config di progetto (`localcoder.json`)
3. Directory plugin globale (`~/.config/localcoder/plugins/`)
4. Directory plugin di progetto (`.localcoder/plugins/`)
...
LocalCoder esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di localcoder per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere localcoder.
...
:::note
Se usi l'app desktop di LocalCoder, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a localcoder di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a localcoder:
...
L'helper `tool` crea uno strumento personalizzato che localcoder può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in localcoder insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://localcoder.ai/docs/sdk) per i dettagli.
