---
title: Server LSP
description: LocalCoder si integra con i tuoi server LSP.
---

LocalCoder si integra con il tuo Language Server Protocol (LSP) per aiutare l'LLM a interagire con la tua codebase. Usa le diagnostiche per fornire feedback all'LLM.

---

## Integrati

LocalCoder include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `LOCALCODER_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando localcoder apre un file:
...
Puoi personalizzare i server LSP tramite la sezione `lsp` nella configurazione di localcoder.
