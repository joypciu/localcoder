---
title: Server
description: Interagisci con il server localcoder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `localcoder serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client localcoder.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `localcoder.local` |
...
Imposta `LOCALCODER_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `localcoder`, oppure imposta `LOCALCODER_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `localcoder serve` sia per `localcoder web`.
...
Quando esegui `localcoder` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server localcoder per interagire con localcoder in modo programmatico.
:::

Questa architettura permette a localcoder di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `localcoder serve` per avviare un server standalone. Se la TUI di localcoder e gia in esecuzione, `localcoder serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di LocalCoder.
...
Il server localcoder espone le seguenti API.
