...
U LocalCoder mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, LocalCoder koristi nasu `localcoder` temu.
...
LocalCoder dolazi sa vise ugradenih tema.
...
- Zele da LocalCoder odgovara izgledu njihovog terminala
...
LocalCoder podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/localcoder/themes/*.json` ili `$XDG_CONFIG_HOME/localcoder/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.localcoder/themes/*.json`
4. **Trenutni radni direktorij** - `./.localcoder/themes/*.json`
