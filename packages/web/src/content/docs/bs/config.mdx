---
title: Konfiguracija
description: Korištenje LocalCoder JSON konfiguracije.
---

Možete konfigurirati LocalCoder koristeći JSON konfiguracijski fajl.

---

## Format

LocalCoder podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/localcoder`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/localcoder/localcoder.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`LOCALCODER_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`localcoder.json` u projektu) - postavke specifične za projekat
5. **`.localcoder` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`LOCALCODER_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.localcoder` i `~/.config/localcoder` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/localcoder` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu LocalCoder konfiguraciju u `~/.config/localcoder/localcoder.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/localcoder/tui.json`.
...
Dodajte `localcoder.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se LocalCoder pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `LOCALCODER_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `LOCALCODER_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.localcoder` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.localcoder`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`localcoder.ai/config.json`**](https://localcoder.ai/config.json).

TUI konfiguracija koristi [**`localcoder.ai/tui.json`**](https://localcoder.ai/tui.json).
...
Koristite `LOCALCODER_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `localcoder.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `localcoder serve` i `localcoder web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš LocalCoder server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `localcoder.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj LocalCoder konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, LocalCoder pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/localcoder/agents/` ili `.localcoder/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, LocalCoder će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`localcoder run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/localcoder/commands/` ili `.localcoder/commands/`. [Saznajte više ovdje](/docs/commands).
...
LocalCoder će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, LocalCoder **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju LocalCoder sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.localcoder/plugins/` ili `~/.config/localcoder/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite LocalCoder da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
