---
title: Server
description: Komunicirajte s localcoder serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `localcoder serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji localcoder klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `localcoder.local` |
...
Postavite `LOCALCODER_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `localcoder`, ili postavite `LOCALCODER_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `localcoder serve` i za `localcoder web`.
...
Kada pokrenete `localcoder`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite localcoder server za programsku interakciju sa localcoder-om.
:::

Ova arhitektura omogucava localcoder podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `localcoder serve` da startate standalone server. Ako je localcoder TUI vec pokrenut, `localcoder serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste LocalCoder [IDE](/docs/ide) pluginovi.
...
localcoder server izlaže sljedece API-je.
