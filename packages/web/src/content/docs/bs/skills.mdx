...
Agent skills omogucavaju LocalCoder da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
LocalCoder pretrazuje ove lokacije:

- Konfiguracija projekta: `.localcoder/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/localcoder/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, LocalCoder ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.localcoder/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/localcoder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.localcoder/skills/git-release/SKILL.md` ovako:
...
LocalCoder navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `localcoder.json`:
...
**Za ugradene agente** (u `localcoder.json`):
