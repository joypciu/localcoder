---
title: Dodaci
description: Napišite vlastite dodatke za proširenje LocalCoder.
---

Dodaci vam omogućavaju da proširite LocalCoder spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja LocalCoder.
...
- `.localcoder/plugins/` - Dodaci na nivou projekta
- `~/.config/localcoder/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/localcoder/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/localcoder/localcoder.json`)
2. Konfiguracija projekta (`localcoder.json`)
3. Globalni direktorij dodataka (`~/.config/localcoder/plugins/`)
4. Direktorij dodataka projekta (`.localcoder/plugins/`)
...
LocalCoder pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: localcoder SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje LocalCoder.
...
Spriječite localcoder da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u localcoder:
...
Pomoćnik `tool` kreira prilagođeni alat koji localcoder može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za localcoder zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://localcoder.ai/docs/sdk) za detalje.
