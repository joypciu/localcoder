...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. LocalCoder dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u LocalCoder.
...
:::note
Ovaj alat je dostupan samo kada je `LOCALCODER_EXPERIMENTAL_LSP_TOOL=true` (ili `LOCALCODER_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz LocalCoder provajdera ili kada je varijabla `LOCALCODER_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju LocalCoder:
