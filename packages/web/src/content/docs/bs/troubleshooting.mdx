...
Da biste otklonili probleme s LocalCoder, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\localcoder\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `localcoder --log-level DEBUG`.

---

## Pohrana

LocalCoder pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/localcoder/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\localcoder`
...
LocalCoder Desktop pokreće lokalni LocalCoder server (`localcoder-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `LocalCoder` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/localcoder/localcoder.jsonc` (ili `~/.config/localcoder/localcoder.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/localcoder/localcoder.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\localcoder\localcoder.jsonc`
...
LocalCoder također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/localcoder/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\localcoder\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga LocalCoder mogao ponovo izgraditi.

1. Potpuno zatvorite LocalCoder Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/localcoder`
- **Linux**: obrišite `~/.cache/localcoder` (ili pokrenite `rm -rf ~/.cache/localcoder`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\localcoder`

3. Ponovo pokrenite LocalCoder Desktop.

---

### Rješavanje problema sa vezom na serveru

LocalCoder Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `localcoder.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `LOCALCODER_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `LOCALCODER_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, LocalCoder Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa LocalCoder karakteristikama.

---

### Obavještenja se ne prikazuju

LocalCoder Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za LocalCoder u postavkama vašeg OS-a, i
...
1. Zatvorite LocalCoder Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka LocalCoder Desktop aplikacije):

- `localcoder.settings.dat` (URL zadanog servera za desktop)
- `localcoder.global.dat` i `localcoder.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s LocalCoder:
...
Da saznate kojim modelima imate pristup, pokrenite `localcoder models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\localcoder`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. LocalCoder dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\localcoder`

2. Ponovo pokrenite LocalCoder da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti LocalCoder da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
LocalCoder će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
