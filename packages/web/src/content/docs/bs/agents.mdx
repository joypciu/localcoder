...
Postoje dvije vrste agenata u LocalCoder: primarni agenti i podagenti.
...
LocalCoder dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
LocalCoder dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

LocalCoder dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `localcoder.json`:
...
- Globalno: `~/.config/localcoder/agents/`
- Po projektu: `.localcoder/agents/`
...
Ako temperatura nije navedena, LocalCoder koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu LocalCoder konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj LocalCoder konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [LocalCoder Zen](/docs/zen), koristili biste `localcoder/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `localcoder models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/localcoder).
:::
