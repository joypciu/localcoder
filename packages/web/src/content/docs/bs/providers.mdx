---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u LocalCoder.
---

import config from "../../../../config.mjs"
export const console = config.console

LocalCoder koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj LocalCoder konfiguraciji.
...
u `~/.local/share/localcoder/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem LocalCoder
config.
...
LocalCoder Zen je lista modela koje je obezbedio LocalCoder tim koji su bili
testirano i potvrđeno da dobro radi sa LocalCoder. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa LocalCoder Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite localcoder i idite na [localcoder.ai/auth](https://localcoder.ai/auth).
...
Radi kao i svaki drugi provajder u LocalCoder i potpuno je opcionalan za korištenje.

---

## LocalCoder Go

LocalCoder Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža LocalCoder tim i koji su testirani i verificirani da dobro rade s LocalCoder-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `LocalCoder Go` i idite na [localcoder.ai/auth](https://localcoder.ai/zen).
...
Da biste koristili Amazon Bedrock s LocalCoder:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete localcoder:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `localcoder.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u LocalCoder.
:::
...
Možete konfigurirati localcoder za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi localcoder ispravno radio.
   :::
...
Također možete dodati modele kroz svoju localcoder konfiguraciju.
...
da pohrani token u localcoder auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
LocalCoder koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate LocalCoder
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`localcoder.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [localcoder-gitlab-auth](https://www.npmjs.com/package/localcoder-gitlab-auth).

##### Konfiguracija

Prilagodite putem `localcoder.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s localcoder:
...
Za korištenje Google Vertex AI s LocalCoder:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće localcoder.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem localcodera, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [localcoder-helicone-session](https://github.com/H2Shami/localcoder-helicone-session) da automatski prijavite svaki LocalCoder razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki LocalCoder razgovor naveden kao zasebna sesija.
...
Možete konfigurirati localcoder za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati localcoder za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati localcoder za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za LocalCoder. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/localcoder) za detalje.
:::
...
Da biste koristili Ollama Cloud s LocalCoder:
...
3. Kopirajte API ključ za korištenje u LocalCoder.
...
6. **Važno**: Prije upotrebe modela oblaka u LocalCoder, morate lokalno povući informacije o modelu:
...
LocalCoder Zen je lista testiranih i verifikovanih modela koju je obezbedio LocalCoder tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>LocalCoder Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **LocalCoder Zen**.

```txt
   /connect
```

3. Unesite svoj LocalCoder API ključ.
...
Također možete dodati dodatne modele putem vaše localcoder konfiguracije.
...
5. Također ih možete prilagoditi putem vaše localcoder konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa localcoderom:
...
Također možete prilagoditi modele kroz svoju localcoder konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s localcoder-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `localcoder.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju LocalCoder da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `localcoder auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite LocalCoder konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj localcoder konfiguraciji.
