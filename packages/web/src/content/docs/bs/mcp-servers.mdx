...
Možete dodati vanjske alate u LocalCoder koristeći _Model Context Protocol_, ili MCP. LocalCoder podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [LocalCoder Config](https://localcoder.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/localcoder`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
LocalCoder automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, LocalCoder će:
...
Ako server zahtijeva autentifikaciju, LocalCoder će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `localcoder mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, LocalCoder sigurno čuva tokene u `~/.local/share/localcoder/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u LocalCoder, zajedno s ugrađenim alatima. Možete njima upravljati kroz LocalCoder konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete LocalCoder sa vašim Sentry nalogom.
