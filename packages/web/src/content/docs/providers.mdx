---
title: Providers
description: Using any LLM provider in LocalCoder.
---

import config from "../../../config.mjs"
export const console = config.console

LocalCoder uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your LocalCoder config.
...
You can customize the providers through the `provider` section in your LocalCoder
config.
...
LocalCoder Zen is a list of models provided by the LocalCoder team that have been
tested and verified to work well with LocalCoder. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with LocalCoder Zen.
:::

1. Run the `/connect` command in the TUI, select `LocalCoder Zen`, and head to [localcoder.ai/auth](https://localcoder.ai/zen).
...
It works like any other provider in LocalCoder and is completely optional to use.

---

## LocalCoder Go

LocalCoder Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the LocalCoder team that have been
tested and verified to work well with LocalCoder.

1. Run the `/connect` command in the TUI, select `LocalCoder Go`, and head to [localcoder.ai/auth](https://localcoder.ai/zen).
...
To use Amazon Bedrock with LocalCoder:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running localcoder:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `localcoder.json`:
...
LocalCoder. Anthropic explicitly prohibits this.

Previous versions of LocalCoder came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in LocalCoder with zero setup:
...
You can configure localcoder to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for localcoder to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your localcoder config.
...
GitLab Duo support in LocalCoder is experimental. Features, configuration, and
behavior may change in future releases.
:::

LocalCoder integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
LocalCoder uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock LocalCoder
to only use your own GitLab-hosted instance, add the following to your
`localcoder.json` file. It is also recommended to disable session sharing.
...
More documentation on [localcoder-gitlab-auth](https://www.npmjs.com/package/localcoder-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, LocalCoder will:
...
4. Route tool execution requests through LocalCoder's permission-gated tool system
...
To use your GitHub Copilot subscription with localcoder:
...
To use Google Vertex AI with LocalCoder:
...
2. Set the required environment variables:
   ...
   Set them while running localcoder.
...
In the event you see a feature or model from Helicone that isn't configured automatically through localcoder, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [localcoder-helicone-session](https://github.com/H2Shami/localcoder-helicone-session) plugin to automatically log each LocalCoder conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each LocalCoder conversation listed as a separate session.
...
You can configure localcoder to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure localcoder to use local models through LM Studio.
...
You can configure localcoder to use local models through Ollama.

:::tip
Ollama can automatically configure itself for LocalCoder. See the [Ollama integration docs](https://docs.ollama.com/integrations/localcoder) for details.
:::
...
To use Ollama Cloud with LocalCoder:
...
3. Copy the API key for use in LocalCoder.
...
6. **Important**: Before using cloud models in LocalCoder, you must pull the model information locally:
...
LocalCoder Zen is a list of tested and verified models provided by the LocalCoder team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>LocalCoder Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **LocalCoder Zen**.

   ```txt
   /connect
   ```

3. Enter your LocalCoder API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your localcoder config.
...
5. You can also customize them through your localcoder config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with localcoder:
...
You can also customize models through your localcoder config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with localcoder. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `localcoder.json` file in your project directory:
...
The `limit` fields allow LocalCoder to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `localcoder auth list` to see if the credentials
...
2. For custom providers, check the localcoder config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your localcoder config.
