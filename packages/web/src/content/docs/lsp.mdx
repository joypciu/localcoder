---
title: LSP Servers
description: LocalCoder integrates with your LSP servers.
---

LocalCoder can integrate with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

LocalCoder comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `LOCALCODER_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and localcoder opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your localcoder config.
...
```json title="localcoder.json" {5-8}
{
  "$schema": "https://localcoder.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="localcoder.json" {5-13}
{
  "$schema": "https://localcoder.ai/config.json",
  "lsp": {
    "custom-lsp": {
