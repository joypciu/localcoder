...
Agent Skills erlauben LocalCoder, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
LocalCoder sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft LocalCoder vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.localcoder/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/localcoder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.localcoder/skills/git-release/SKILL.md` zum Beispiel so:
...
LocalCoder listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `localcoder.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `localcoder.json`):
