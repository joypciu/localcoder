...
Mit LocalCoder kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet LocalCoder das `localcoder`-Theme.
...
LocalCoder liefert mehrere eingebaute Themes mit.
...
- LocalCoder optisch ans Terminal anpassen wollen
...
LocalCoder unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/localcoder/themes/*.json` oder `$XDG_CONFIG_HOME/localcoder/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.localcoder/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.localcoder/themes/*.json`
