...
description: Erstellen Sie Tools, die der LLM in LocalCoder aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von LocalCoder wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.localcoder/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/localcoder/tools/` platzieren.
