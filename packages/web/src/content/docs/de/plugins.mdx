---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um LocalCoder zu erweitern.
---

Mit Plugins können Sie LocalCoder erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von LocalCoder zu ändern.
...
- `.localcoder/plugins/` – Plugins auf Projektebene
- `~/.config/localcoder/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/localcoder/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/localcoder/localcoder.json`)
2. Projektkonfiguration (`localcoder.json`)
3. Globales Plugin-Verzeichnis (`~/.config/localcoder/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.localcoder/plugins/`)
...
LocalCoder führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein LocalCoder-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie LocalCoder erweitern können.
...
:::note
Wenn Sie die LocalCoder-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass LocalCoder `.env`-Dateien liest:
...
Plugins können LocalCoder auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das LocalCoder aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für LocalCoder zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://localcoder.ai/docs/sdk).
