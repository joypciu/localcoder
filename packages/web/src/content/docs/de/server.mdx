---
title: Server
description: Nutze den localcoder-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `localcoder serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein localcoder-Client nutzen kann.
...
Setze `LOCALCODER_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `localcoder`, kann aber mit `LOCALCODER_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `localcoder serve` und `localcoder web`.
...
Wenn du `localcoder` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den localcoder-Server, um localcoder programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `localcoder serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `localcoder serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die LocalCoder-[IDE](/docs/ide)-Plugins.
...
Der localcoder-Server stellt folgende APIs bereit.
