---
title: Regeln
description: Lege eigene Anweisungen fuer localcoder fest.
---

Du kannst localcoder ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in localcoder den Befehl `/init` aus.
...
So kann localcoder sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht localcoder sie zu erweitern.
...
localcoder kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/localcoder/AGENTS.md` speichern.
Diese gelten in allen localcoder-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt LocalCoder dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/localcoder/AGENTS.md` existiert)
...
Beim Start sucht localcoder in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/localcoder/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `localcoder.json` oder in `~/.config/localcoder/localcoder.json` angeben.
...
localcoder parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit localcoder.json

Empfohlen ist das `instructions`-Feld in der `localcoder.json`:
...
Du kannst localcoder explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `localcoder.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
