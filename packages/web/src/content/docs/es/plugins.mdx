---
title: Complementos
description: Escriba sus propios complementos para extender LocalCoder.
---

Los complementos le permiten extender LocalCoder al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de LocalCoder.
...
- `.localcoder/plugins/` - Complementos a nivel de proyecto
- `~/.config/localcoder/plugins/` - Complementos globales
...
```json title="localcoder.json"
{
  ...
  "plugin": ["localcoder-helicone-session", "localcoder-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/localcoder/node_modules/`.
...
1. Configuración global (`~/.config/localcoder/localcoder.json`)
2. Configuración del proyecto (`localcoder.json`)
3. Directorio global de complementos (`~/.config/localcoder/plugins/`)
4. Directorio de complementos del proyecto (`.localcoder/plugins/`)
...
```json title=".localcoder/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
LocalCoder ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".localcoder/plugins/my-plugin.ts"
...
```js title=".localcoder/plugins/example.js"
...
- `client`: Un cliente SDK localcoder para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar localcoder.
...
```js title=".localcoder/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "localcoder"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio LocalCoder, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que localcoder lea archivos `.env`:

```javascript title=".localcoder/plugins/env-protection.js"
...
```javascript title=".localcoder/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a localcoder:

```ts title=".localcoder/plugins/custom-tools.ts"
import { type Plugin, tool } from "@localcoder-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que localcoder puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para localcoder junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://localcoder.ai/docs/sdk) para obtener más detalles.
...
```ts title=".localcoder/plugins/compaction.ts"
import type { Plugin } from "@localcoder-ai/plugin"
...
```ts title=".localcoder/plugins/custom-compaction.ts"
import type { Plugin } from "@localcoder-ai/plugin"
