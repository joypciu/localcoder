...
description: Cree herramientas que LLM pueda llamar en localcoder.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de localcoder como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.localcoder/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/localcoder/tools/`.
...
```ts title=".localcoder/tools/database.ts" {1}
...
```ts title=".localcoder/tools/math.ts"
import { tool } from "@localcoder-ai/plugin"
...
```ts title=".localcoder/tools/bash.ts"
import { tool } from "@localcoder-ai/plugin"
...
```ts title=".localcoder/tools/project.ts" {8}
import { tool } from "@localcoder-ai/plugin"
...
```python title=".localcoder/tools/add.py"
import sys
...
```ts title=".localcoder/tools/python-add.ts" {10}
import { tool } from "@localcoder-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".localcoder/tools/add.py")
