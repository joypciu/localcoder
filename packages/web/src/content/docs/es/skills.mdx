...
Las habilidades del agente permiten a LocalCoder descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
LocalCoder busca estas ubicaciones:

- Configuración del proyecto: `.localcoder/skills/<name>/SKILL.md`
- Configuración global: `~/.config/localcoder/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, LocalCoder sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.localcoder/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/localcoder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.localcoder/skills/git-release/SKILL.md` así:
...
LocalCoder enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `localcoder.json`:
...
**Para agentes integrados** (en `localcoder.json`):
