---
title: Servidor
description: Interactuar con el servidor localcoder a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `localcoder serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente localcoder puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `localcoder.local` |
...
Configure `LOCALCODER_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `localcoder`, o configure `LOCALCODER_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `localcoder serve` como a `localcoder web`.
...
Cuando ejecuta `localcoder`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor localcoder para interactuar con localcoder mediante programación.
:::

Esta arquitectura permite que localcoder admita múltiples clientes y le permite interactuar con localcoder mediante programación.

Puede ejecutar `localcoder serve` para iniciar un servidor independiente. Si tienes el
localcoder TUI ejecutándose, `localcoder serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos LocalCoder [IDE](/docs/ide).
...
El servidor localcoder expone las siguientes API.
