...
Las herramientas permiten que LLM realice acciones en su código base. LocalCoder viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en LocalCoder.
...
:::note
Esta herramienta solo está disponible cuando `LOCALCODER_EXPERIMENTAL_LSP_TOOL=true` (o `LOCALCODER_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor LocalCoder o cuando la variable de entorno `LOCALCODER_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar LocalCoder:

```bash
LOCALCODER_ENABLE_EXA=1 localcoder
```
