...
Con LocalCoder puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, LocalCoder usa nuestro propio tema `localcoder`.
...
LocalCoder viene con varios temas integrados.
...
- Quiere que LocalCoder coincida con la apariencia de su terminal
...
LocalCoder admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/localcoder/themes/*.json` o `$XDG_CONFIG_HOME/localcoder/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.localcoder/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.localcoder/themes/*.json`
