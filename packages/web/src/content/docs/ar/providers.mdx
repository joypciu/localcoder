---
title: المزوّدون
description: استخدام أي موفّر LLM في LocalCoder.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم LocalCoder ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات LocalCoder.
...
في `~/.local/share/localcoder/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات LocalCoder.
...
LocalCoder Zen هي قائمة نماذج يوفّرها فريق LocalCoder وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع LocalCoder. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع LocalCoder Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر localcoder، ثم انتقل إلى [localcoder.ai/auth](https://localcoder.ai/auth).
...
يعمل مثل أي مزوّد آخر في LocalCoder واستخدامه اختياري بالكامل.

---

## LocalCoder Go

LocalCoder Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق LocalCoder، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع LocalCoder.

1. شغّل الأمر `/connect` في TUI، واختر `LocalCoder Go`، ثم انتقل إلى [localcoder.ai/auth](https://localcoder.ai/zen).
...
لاستخدام Amazon Bedrock مع LocalCoder:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل localcoder:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `localcoder.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل LocalCoder غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين localcoder لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل localcoder بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات localcoder.
...
تخزين الرمز في مساحة تخزين مصادقة localcoder.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم LocalCoder نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر LocalCoder
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`localcoder.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [localcoder-gitlab-auth](https://www.npmjs.com/package/localcoder-gitlab-auth).

##### التهيئة

خصّص عبر `localcoder.json`:
...
لاستخدام اشتراك GitHub Copilot مع localcoder:
...
لاستخدام Google Vertex AI مع LocalCoder:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر localcoder، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [localcoder-helicone-session](https://github.com/H2Shami/localcoder-helicone-session) لتسجيل كل محادثة LocalCoder تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة LocalCoder مدرجة كجلسة مستقلة.
...
يمكنك ضبط localcoder لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط localcoder لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط localcoder لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع LocalCoder. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/localcoder) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع LocalCoder:
...
3. انسخ مفتاح API لاستخدامه في LocalCoder.
...
6. **مهم**: قبل استخدام نماذج السحابة في LocalCoder، يجب سحب معلومات النموذج محليا:
...
LocalCoder Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق LocalCoder. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>LocalCoder Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **LocalCoder Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ LocalCoder.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات localcoder.
...
5. يمكنك أيضا تخصيصها عبر إعدادات localcoder. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع localcoder:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات localcoder. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع localcoder. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `localcoder.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ LocalCoder بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `localcoder auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات localcoder وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات localcoder.
