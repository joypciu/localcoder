...
أنشئ الملف `.localcoder/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات LocalCoder أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ LocalCoder:
...
- على مستوى النظام: `~/.config/localcoder/commands/`
- لكل مشروع: `.localcoder/commands/`
...
يتضمن localcoder عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
