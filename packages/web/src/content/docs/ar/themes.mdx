...
مع LocalCoder يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم LocalCoder سمتنا `localcoder`.
...
يأتي LocalCoder مع عدة سمات مدمجة.
...
- يريدون أن يطابق LocalCoder مظهر terminal لديهم
...
يدعم LocalCoder نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/localcoder/themes/*.json` أو `$XDG_CONFIG_HOME/localcoder/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.localcoder/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.localcoder/themes/*.json`
