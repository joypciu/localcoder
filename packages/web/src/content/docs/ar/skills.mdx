...
تتيح مهارات الوكيل لـ LocalCoder اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث LocalCoder في هذه المواقع:

- إعدادات المشروع: `.localcoder/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/localcoder/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج LocalCoder صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.localcoder/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/localcoder/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.localcoder/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض LocalCoder المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `localcoder.json`:
...
**للوكلاء المدمجين** (في `localcoder.json`):
