...
يمكنك إضافة أدوات خارجية إلى LocalCoder باستخدام _Model Context Protocol_، أو MCP. يدعم LocalCoder خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات LocalCoder](https://localcoder.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/localcoder` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى LocalCoder تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم LocalCoder بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك LocalCoder المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `localcoder mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن LocalCoder الرموز بشكل آمن في `~/.local/share/localcoder/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل LocalCoder إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات LocalCoder مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط LocalCoder بحساب Sentry الخاص بك.
