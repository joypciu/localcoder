---
title: الخادم
description: تفاعل مع خادم localcoder عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `localcoder serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل localcoder استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `localcoder.local` |
...
عيّن `LOCALCODER_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `localcoder`، أو عيّن `LOCALCODER_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `localcoder serve` و `localcoder web`.
...
عند تشغيل `localcoder` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم localcoder للتفاعل مع localcoder برمجيا.
:::

تتيح هذه البنية لـ localcoder دعم عدة عملاء وتمكّنك من التفاعل مع localcoder برمجيا.

يمكنك تشغيل `localcoder serve` لبدء خادم مستقل. إذا كانت واجهة localcoder في terminal (TUI)
قيد التشغيل، فسيبدأ `localcoder serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات LocalCoder لـ [IDE](/docs/ide).
...
يعرض خادم localcoder واجهات API التالية.
