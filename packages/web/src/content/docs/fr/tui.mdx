---
title: TUI
description: Utilisation de l’interface utilisateur du terminal LocalCoder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LocalCoder fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de LocalCoder démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le LocalCoder TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à LocalCoder. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez LocalCoder. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `localcoder.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `LOCALCODER_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.
