...
Avec LocalCoder, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, LocalCoder utilise notre propre thème `localcoder`.
...
LocalCoder est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que LocalCoder corresponde à l'apparence de leur terminal
...
LocalCoder prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/localcoder/themes/*.json` ou `$XDG_CONFIG_HOME/localcoder/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.localcoder/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.localcoder/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/localcoder/themes
vim ~/.config/localcoder/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .localcoder/themes
vim .localcoder/themes/my-theme.json
```
