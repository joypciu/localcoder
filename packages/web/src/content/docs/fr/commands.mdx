...
Créez `.localcoder/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration LocalCoder ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre LocalCoder [config](/docs/config) :
...
- Par projet : `.localcoder/commands/`
...
localcoder comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
