---
title: Plugins
description: Écrivez vos propres plugins pour étendre LocalCoder.
---

Les plugins vous permettent d'étendre LocalCoder en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de LocalCoder.
...
- `.localcoder/plugins/` - Plugins au niveau du projet
- `~/.config/localcoder/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/localcoder/node_modules/`.
...
1. Configuration globale (`~/.config/localcoder/localcoder.json`)
2. Configuration du projet (`localcoder.json`)
3. Répertoire global des plugins (`~/.config/localcoder/plugins/`)
4. Répertoire des plugins du projet (`.localcoder/plugins/`)
...
LocalCoder exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK localcoder pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre localcoder.
...
:::note
Si vous utilisez l'application de bureau LocalCoder, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez localcoder de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à localcoder :
...
L'assistant `tool` crée un outil personnalisé que localcoder peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour localcoder aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://localcoder.ai/docs/sdk) pour plus de détails.
