...
Il existe deux types d'agents dans LocalCoder ; agents primaires et sous-agents.
...
LocalCoder est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
LocalCoder est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

LocalCoder est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `localcoder.json` :
...
- Global : `~/.config/localcoder/agents/`
- Par projet : `.localcoder/agents/`

```markdown title="~/.config/localcoder/agents/review.md"
---
...
Si aucune température n'est spécifiée, LocalCoder utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale LocalCoder et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration LocalCoder utilise le format `provider/model-id`. Par exemple, si vous utilisez [LocalCoder Zen](/docs/zen), vous utiliserez `localcoder/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="localcoder.json" {3-6,9-12}
{
  "$schema": "https://localcoder.ai/config.json",
  "tools": {
    "write": true,
...
```json title="localcoder.json" {3-5,8-10}
{
  "$schema": "https://localcoder.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="localcoder.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Exécutez `localcoder models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/localcoder).
:::
...
```markdown title="~/.config/localcoder/agents/docs-writer.md"
---
...
```markdown title="~/.config/localcoder/agents/security-auditor.md"
---
