---
title: Serveur
description: Interagissez avec le serveur localcoder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `localcoder serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client localcoder peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `localcoder.local` |
...
Définissez `LOCALCODER_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `localcoder`, ou définissez `LOCALCODER_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `localcoder serve` et à `localcoder web`.
...
Lorsque vous exécutez `localcoder`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur localcoder pour interagir avec localcoder par programmation.
:::

Cette architecture permet à localcoder de prendre en charge plusieurs clients et vous permet d'interagir avec localcoder par programme.

Vous pouvez exécuter `localcoder serve` pour démarrer un serveur autonome. Si vous avez le
localcoder TUI en cours d'exécution, `localcoder serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins LocalCoder [IDE](/docs/ide).
...
Le serveur localcoder expose les API suivantes.
