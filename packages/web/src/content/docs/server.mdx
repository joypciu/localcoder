---
title: Server
description: Interact with localcoder server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `localcoder serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an localcoder client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `localcoder.local` |
...
Set `LOCALCODER_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `localcoder`, or set `LOCALCODER_SERVER_USERNAME` to override it. This applies to both `localcoder serve` and `localcoder web`.
...
When you run `localcoder` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the localcoder server to interact with localcoder programmatically.
:::

This architecture lets localcoder support multiple clients and allows you to interact with localcoder programmatically.

You can run `localcoder serve` to start a standalone server. If you have the
localcoder TUI running, `localcoder serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the LocalCoder [IDE](/docs/ide) plugins.
...
The localcoder server exposes the following APIs.
