---
title: Plugins
description: Skriv dine egne plugins for at udvide LocalCoder.
---

Plugins giver dig mulighed for at udvide LocalCoder ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre LocalCoders standardadfærd.
...
- `.localcoder/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/localcoder/node_modules/`.
...
1. Global konfiguration (`~/.config/localcoder/localcoder.json`)
2. Projektkonfiguration (`localcoder.json`)
3. Global plugin-mappe (`~/.config/localcoder/plugins/`)
4. Projekt plugin bibliotek (`.localcoder/plugins/`)
...
LocalCoder kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En localcoder SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide localcoder.
...
:::note
Hvis du bruger LocalCoder desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå localcoder i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til localcoder:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som localcoder kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for localcoder sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://localcoder.ai/docs/sdk) for detaljer.
