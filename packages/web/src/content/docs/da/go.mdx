...
LocalCoder Go er et lavprisabonnement — **$5 for din første måned**, derefter **$10/måned** — der giver dig pålidelig adgang til populære åbne kodningsmodeller.

:::note
LocalCoder Go er i øjeblikket i beta.
:::

Go fungerer som enhver anden udbyder i LocalCoder. Du abonnerer på LocalCoder Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med LocalCoder.
:::
...
LocalCoder Go giver dig adgang til disse modeller for **$5 for din første måned**, derefter **$10/måned**.

---

## Sådan fungerer det

LocalCoder Go fungerer som enhver anden udbyder i LocalCoder.

1. Du logger ind på **<a href={console}>LocalCoder Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `LocalCoder Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på LocalCoder Go.
:::
...
LocalCoder Go inkluderer følgende grænser:
...
Dit [model id](/docs/config/#models) i din LocalCoder config
bruger formatet `localcoder-go/<model-id>`. For eksempel for Kimi K2.6, vil du
bruge `localcoder-go/kimi-k2.6` i din config.
...
Vi skabte LocalCoder Go for at:
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med LocalCoder.
