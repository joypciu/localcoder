---
title: Server
description: Interager med localcoder-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `localcoder serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en localcoder-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `localcoder.local` |
...
Indstil `LOCALCODER_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `localcoder`, eller indstil `LOCALCODER_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `localcoder serve` og `localcoder web`.
...
Når du kører `localcoder` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug localcoder-serveren til at interagere med localcoder programmatisk.
:::

Denne arkitektur lader localcoder understøtte flere klienter og lader dig interagere med localcoder programmatisk.

Du kan køre `localcoder serve` for at starte en selvstændig server. Hvis du har
localcoder TUI kørende, vil `localcoder serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af LocalCoder [IDE](/docs/ide) plugins.
...
LocalCoder-serveren viser følgende API'er.
