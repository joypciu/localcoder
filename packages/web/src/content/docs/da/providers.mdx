---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i LocalCoder.
---

import config from "../../../../config.mjs"
export const console = config.console

LocalCoder bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i LocalCoder-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/localcoder/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i LocalCoder-konfigurationen.
...
LocalCoder Zen er en liste over modeller leveret af LocalCoder-teamet, som er blevet testet og verificeret til at fungere godt med LocalCoder. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med LocalCoder Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg localcoder og gå til [localcoder.ai/auth](https://localcoder.ai/auth).
...
Det fungerer som alle andre udbydere i LocalCoder og er helt valgfrit at bruge.

---

## LocalCoder Go

LocalCoder Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af LocalCoder-teamet, som er testet og verificeret til at fungere godt med LocalCoder.

1. Kør kommandoen `/connect` i TUI, vælg `LocalCoder Go`, og gå til [localcoder.ai/auth](https://localcoder.ai/zen).
...
Sådan bruger du Amazon Bedrock med LocalCoder:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører localcoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `localcoder.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i LocalCoder understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere localcoder til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at localcoder skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem localcoder-konfigurationen.
...
gemme tokenet i localcoder auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
LocalCoder bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse LocalCoder
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`localcoder.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [localcoder-gitlab-auth](https://www.npmjs.com/package/localcoder-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `localcoder.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med LocalCoder:
...
Sådan bruger du Google Vertex AI med LocalCoder:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører localcoder.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem localcoder, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [localcoder-helicone-session](https://github.com/H2Shami/localcoder-helicone-session) plugin for automatisk at logge hver LocalCoder-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver LocalCoder-samtale opført som en separat session.
...
Du kan konfigurere localcoder til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere localcoder til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere localcoder til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for LocalCoder. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/localcoder) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med LocalCoder:
...
3. Kopier API-nøglen til brug i LocalCoder.
...
6. **Vigtigt**: Før du bruger skymodeller i LocalCoder, skal du hente modelinformationen lokalt:
...
LocalCoder Zen er en liste over testede og verificerede modeller leveret af LocalCoder-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>LocalCoder Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **LocalCoder Zen**.

   ```txt
   /connect
   ```

3. Indtast LocalCoder API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem localcoder-konfigurationen.
...
5. Du kan også tilpasse dem gennem localcoder-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med LocalCoder:
...
Du kan også tilpasse modeller gennem localcoder-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med localcoder. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `localcoder.json`-filen i projektkataloget:
...
`limit`-felterne lader LocalCoder forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `localcoder auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek localcoder-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i localcoder-konfigurationen.
