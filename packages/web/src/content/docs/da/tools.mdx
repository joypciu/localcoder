...
Her er alle de innebygde verktøyene som er tilgængelige i LocalCoder.
...
:::note
Dette verktøyet er bare tilgængelig når `LOCALCODER_EXPERIMENTAL_LSP_TOOL=true` (eller `LOCALCODER_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger LocalCoder-leverandøren eller når miljøvariabelen `LOCALCODER_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
