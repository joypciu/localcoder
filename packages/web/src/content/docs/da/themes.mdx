...
Med LocalCoder kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger LocalCoder vårt eget `localcoder`-tema.
...
LocalCoder kommer med flere innebygde temaer.
...
- Vil at LocalCoder skal matche terminalens utseende
...
LocalCoder støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/localcoder/themes/*.json` eller `$XDG_CONFIG_HOME/localcoder/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.localcoder/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.localcoder/themes/*.json`
