...
LocalCoder Goは低価格のサブスクリプションで、**初月は5ドル**、その後は**月額10ドル**で、人気のオープンなコーディングモデルに安定してアクセスできます。

:::note
LocalCoder Goは現在ベータ版です。
:::

GoはLocalCoderの他のプロバイダーと同様に機能します。LocalCoder GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、LocalCoderを使用するために必須ではありません。
...
:::tip
LocalCoderでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
LocalCoder Goを使用すると、これらのモデルに**初月は5ドル**、その後は**月額10ドル**でアクセスできます。

---

## 仕組み

LocalCoder Goは、LocalCoderの他のプロバイダーと同様に機能します。

1. **<a href={console}>LocalCoder Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`LocalCoder Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
LocalCoder Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
LocalCoder設定の[model id](/docs/config/#models)は、`localcoder-go/<model-id>`という形式を使用します。たとえば、Kimi K2.6の場合は、設定で`localcoder-go/kimi-k2.6`を使用します。
...
4. LocalCoderで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
