---
title: プラグイン
description: LocalCoder を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで LocalCoder を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、LocalCoder のデフォルトの動作を変更したりできます。
...
- `.localcoder/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/localcoder/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/localcoder/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/localcoder/localcoder.json`)
2. プロジェクト設定 (`localcoder.json`)
3. グローバルプラグインディレクトリ (`~/.config/localcoder/plugins/`)
4. プロジェクトプラグインディレクトリ (`.localcoder/plugins/`)
...
LocalCoder は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための LocalCoder SDK クライアント。
...
ここでは、LocalCoder を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://localcoder.ai/docs/sdk) を参照してください。
...
LocalCoder が `.env` ファイルを読み取らないようにします。
...
プラグインは LocalCoder にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、LocalCoder が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して LocalCoder で使用できます。
