---
title: サーバー
description: HTTP 経由で LocalCoder サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`localcoder serve` コマンドは、LocalCoder クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `localcoder.local` |
...
HTTP 基本認証でサーバーを保護するには、`LOCALCODER_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `localcoder` になるか、`LOCALCODER_SERVER_USERNAME` を設定してオーバーライドします。これは、`localcoder serve` と `localcoder web` の両方に当てはまります。
...
`localcoder` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
LocalCoder サーバーを使用して、プログラムで LocalCoder と対話します。
:::

`localcoder serve` を実行してスタンドアロンサーバーを起動できます。すでに LocalCoder TUI を実行している場合でも、`localcoder serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、LocalCoder [IDE](/docs/ide) プラグインによって使用されます。
