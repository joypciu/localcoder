---
title: TUI
description: Using the LocalCoder terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LocalCoder provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running LocalCoder starts the TUI for the current directory.
...
When using the LocalCoder TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to LocalCoder. Allows you to select from available providers and add their API keys.
...
Exit LocalCoder. _Aliases_: `/quit`, `/q`
...
This is separate from `localcoder.json`, which configures server/runtime behavior.
...
Use `LOCALCODER_TUI_CONFIG` to load a custom TUI config path.
