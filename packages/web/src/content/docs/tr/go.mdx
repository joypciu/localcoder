...
LocalCoder Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
LocalCoder Go şu anda beta olarak mevcut.
:::

Go, LocalCoder'daki diğer sağlayıcılar gibi çalışır. LocalCoder Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve LocalCoder'u kullanmak için bunu kullanmanıza gerek yoktur.
...
LocalCoder Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

LocalCoder Go, LocalCoder'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>LocalCoder Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `LocalCoder Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye LocalCoder Go'ya abone olabilir.
:::
...
LocalCoder Go aşağıdaki limitleri içerir:
...
LocalCoder yapılandırmanızdaki [model id](/docs/config/#models) formatı `localcoder-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `localcoder-go/kimi-k2.6` kullanmalısınız.
...
LocalCoder Go'yu şu amaçlarla oluşturduk:
...
4. LocalCoder ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
