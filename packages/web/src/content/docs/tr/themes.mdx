...
localcoder ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak localcoder kendi `localcoder` temasını kullanır.
...
localcoder birden fazla yerleşik temayla gelir.
...
- localcoder'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
localcoder, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/localcoder/themes/*.json` veya `$XDG_CONFIG_HOME/localcoder/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.localcoder/themes/*.json`
4. **Geçerli çalışma dizini** - `./.localcoder/themes/*.json`
