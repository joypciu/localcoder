...
description: LocalCoder, LSP sunucularınızla bütünleşir.
---

LocalCoder, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

LocalCoder, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`LOCALCODER_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
localcoder bir dosyayı açtığında:
...
LSP sunucularını, localcoder yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
