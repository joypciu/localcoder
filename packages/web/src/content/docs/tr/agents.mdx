...
localcoder'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
localcoder, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
localcoder, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

localcoder iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `localcoder.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.localcoder/agents/`
...
Sıcaklık belirtilmezse localcoder modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global localcoder yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
localcoder yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [LocalCoder Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `localcoder/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `localcoder models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/localcoder).
:::
