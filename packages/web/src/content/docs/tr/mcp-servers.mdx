...
_Model Bağlam Protokolü_ veya MCP kullanarak localcoder'a harici araçlar ekleyebilirsiniz. localcoder hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [localcoder Config](https://localcoder.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/localcoder` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
localcoder, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde localcoder şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa localcoder, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `localcoder mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından localcoder, belirteçleri `~/.local/share/localcoder/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra localcoder'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi localcoder yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve localcoder'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
