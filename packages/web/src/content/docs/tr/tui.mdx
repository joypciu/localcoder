---
title: TUI
description: LocalCoder terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LocalCoder, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

LocalCoder'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
LocalCoder TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
LocalCoder'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
LocalCoder'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `localcoder.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `LOCALCODER_TUI_CONFIG` kullanın.
