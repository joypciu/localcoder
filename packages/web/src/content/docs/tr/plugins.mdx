---
title: Eklentiler
description: LocalCoder'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek LocalCoder'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya LocalCoder'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.localcoder/plugins/` - Proje düzeyinde eklentiler
- `~/.config/localcoder/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/localcoder/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/localcoder/localcoder.json`)
2. Project config (`localcoder.json`)
3. Global eklenti dizini (`~/.config/localcoder/plugins/`)
4. Proje eklenti dizini (`.localcoder/plugins/`)
...
LocalCoder bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir LocalCoder SDK istemcisi.
...
LocalCoder'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
LocalCoder masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
LocalCoder'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca LocalCoder'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, LocalCoder'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra LocalCoder'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://localcoder.ai/docs/sdk) bakın.
