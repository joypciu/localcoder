---
title: Sağlayıcılar
description: localcoder'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

localcoder, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı localcoder yapılandırmanızda yapılandırın.
...
`~/.local/share/localcoder/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları localcoder'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
LocalCoder Zen, localcoder ekibi tarafından sağlanan modellerin bir listesidir.
localcoder ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz LocalCoder Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, localcoder'u seçin ve [localcoder.ai/auth](https://localcoder.ai/auth)'ye gidin.
...
localcoder'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## LocalCoder Go

LocalCoder Go, localcoder ile iyi çalıştığı test edilmiş ve doğrulanmış, localcoder ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `LocalCoder Go`'yu seçin ve [localcoder.ai/auth](https://localcoder.ai/zen) adresine gidin.
...
Amazon Bedrock'u localcoder ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   localcoder'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `localcoder.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi localcoder'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
localcoder'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   localcoder'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   localcoder yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in localcoder auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
localcoder, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. localcoder'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`localcoder.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [localcoder-gitlab-auth](https://www.npmjs.com/package/localcoder-gitlab-auth) ana sayfasında.

##### Yapılandırma

`localcoder.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi localcoder'la kullanmak için:
...
Google Vertex AI'yi localcoder ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları localcoder çalıştırılırken ayarlayın.
...
Helicone'un localcoder aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her localcoder görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [localcoder-helicone-session](https://github.com/H2Shami/localcoder-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her localcoder görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
localcoder'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla localcoder'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla localcoder'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini localcoder için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/localcoder)'a bakın.
:::
...
Ollama Cloud'u localcoder ile kullanmak için:
...
3. localcoder'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: localcoder'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
LocalCoder Zen, localcoder ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>LocalCoder Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **LocalCoder Zen**'i arayın.

   ```txt
   /connect
   ```

3. localcoder API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   localcoder yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca localcoder yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi localcoder ile kullanmak için:
...
Modelleri localcoder yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı localcoder ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `localcoder.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları localcoder'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `localcoder auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için localcoder yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, localcoder yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
