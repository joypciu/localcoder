---
title: Sunucu
description: localcoder sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`localcoder serve` komutu, localcoder istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `localcoder.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `LOCALCODER_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `localcoder` değeridir; değiştirmek isterseniz `LOCALCODER_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `localcoder serve` hem de `localcoder web` için geçerlidir.
...
`localcoder` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
localcoder ile programatik etkileşim için localcoder sunucusunu kullanın.
:::

Bu mimari, localcoder'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`localcoder serve` ile bağımsız bir sunucu başlatabilirsiniz. localcoder TUI açıksa, `localcoder serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum localcoder [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
localcoder sunucusu aşağıdaki API'leri sunar.
