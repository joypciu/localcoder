...
Dzięki localcoder możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie localcoder używa naszego własnego motywu `localcoder`.
...
localcoder ma kilka wbudowanych motywów.
...
- Chcą, aby LocalCoder pasował do wyglądu ich terminala
...
localcoder obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/localcoder/themes/*.json` lub `$XDG_CONFIG_HOME/localcoder/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.localcoder/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.localcoder/themes/*.json`
