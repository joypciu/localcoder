---
title: Konfiguracja
description: Ustawienia LocalCoder w pliku JSON.
---

Możesz dostosować LocalCoder za pomocą pliku konfiguracyjnego JSON.

---

## Format

LocalCoder obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "localcoder"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/localcoder`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/localcoder/localcoder.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `LOCALCODER_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`localcoder.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.localcoder`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `LOCALCODER_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.localcoder` i `~/.config/localcoder` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/localcoder`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację LocalCoder w `~/.config/localcoder/localcoder.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/localcoder/tui.json`.
...
Dodaj `localcoder.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy LocalCoder się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `LOCALCODER_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `LOCALCODER_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.localcoder` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.localcoder`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`localcoder.ai/config.json`**](https://localcoder.ai/config.json).
...
Użyj `LOCALCODER_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `localcoder.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `localcoder serve` i `localcoder web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer LocalCoder.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `localcoder.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie LocalCoder wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/localcoder/agents/` lub `.localcoder/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, LocalCoder powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`localcoder run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/localcoder/commands/` lub `.localcoder/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
LocalCoder może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy LocalCoder nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie LocalCoder **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają LocalCoder o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.localcoder/plugins/` lub `~/.config/localcoder/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie LocalCoder tylko do określonych dostawców.
