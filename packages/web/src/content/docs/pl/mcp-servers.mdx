...
Możesz dodać dodatkowe informacje do localcoder za pomocą _Model Context Protocol_, w skrócie MCP. localcoder obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [localcoder Config](https://localcoder.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/localcoder`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
localcoder automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, localcoder:
...
Jeśli serwer wymaga uwierzytelnienia, localcoder poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `localcoder mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji localcoder bezpieczne przechowa tokeny w `~/.local/share/localcoder/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w localcoder, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie localcoder, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć localcoder z kontem Sentry.
