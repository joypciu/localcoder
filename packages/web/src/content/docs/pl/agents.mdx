...
W LocalCoder obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
LocalCoder zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
LocalCoder ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

LocalCoder ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `localcoder.json`:
...
- Globalnie: `~/.config/localcoder/agents/`
- W projekcie: `.localcoder/agents/`
...
Jeśli nie zostanie skonfigurowana, LocalCoder używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji LocalCoder, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji LocalCoder w `provider/model-id`. Na przykład, [LocalCoder Zen](/docs/zen), użyj `localcoder/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `localcoder models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/localcoder).
:::
