---
title: Serwer
description: Interakcja z serwerem localcoder za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `localcoder serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient localcoder.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `localcoder.local` |
...
Ustaw `LOCALCODER_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `localcoder` lub ustaw `LOCALCODER_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `localcoder serve`, jak i `localcoder web`.
...
Kiedy uruchomisz `localcoder`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera localcoder do programowej interakcji z localcoder.
:::

Ta architektura umożliwia obsługę wielu klientów przez localcoder i programową interakcję z kodem otwartym.

Możesz uruchomić `localcoder serve`, aby uruchomić samodzielny serwer. Jeśli masz
localcoder TUI działa, `localcoder serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki localcoder [IDE](/docs/ide).
...
Serwer localcoder udostępnia następujące interfejsy API.
