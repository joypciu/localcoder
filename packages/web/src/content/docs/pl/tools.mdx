...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. LocalCoder posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w LocalCoder.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `LOCALCODER_EXPERIMENTAL_LSP_TOOL=true` (lub `LOCALCODER_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy LocalCoder lub gdy zmienna środowiskowa `LOCALCODER_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu LocalCoder:
