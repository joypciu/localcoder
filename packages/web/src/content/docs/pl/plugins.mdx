---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć localcoder.
---

Wtyczki umożliwiające wyłączenie localcoder poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania localcoder.
...
- `.localcoder/plugins/` - Wtyczki na poziomie projektu
- `~/.config/localcoder/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/localcoder/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/localcoder/localcoder.json`)
2. Konfiguracja projektu (`localcoder.json`)
3. Globalny katalog wtyczek (`~/.config/localcoder/plugins/`)
4. Katalog wtyczek projektu (`.localcoder/plugins/`)
...
localcoder uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z localcoder do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do localcoder.
...
:::note
Jeśli korzystasz z aplikacji komputerowej localcoder, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj localcoder czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do localcoder:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod localcoder. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://localcoder.ai/docs/sdk).
