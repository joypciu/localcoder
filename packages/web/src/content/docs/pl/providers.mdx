---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w localcoder.
---

import config from "../../../../config.mjs"
export const console = config.console

localcoder używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji localcoder.
...
w `~/.local/share/localcoder/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim localcoder
config.
...
LocalCoder Zen to lista modeli dostarczonych przez zespół localcoder, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z localcoder. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od LocalCoder Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `LocalCoder Zen` i przejdź do [localcoder.ai/auth](https://localcoder.ai/zen).
...
Działa jak każdy inny dostawca w localcoder i jest całkowicie opcjonalny w użyciu.

---

## LocalCoder Go

LocalCoder Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół localcoder, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z localcoder.

1. Uruchom polecenie `/connect` w TUI, wybierz `LocalCoder Go` i przejdź do [localcoder.ai/auth](https://localcoder.ai/zen).
...
Aby używać Amazon Bedrock z localcoder:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania localcoder:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `localcoder.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w localcoder nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować localcoder do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod localcoder działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji localcoder.
...
localcoder używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować localcoder
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`localcoder.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [localcoder-gitlab-auth](https://www.npmjs.com/package/localcoder-gitlab-auth).

##### Konfiguracja

Customize through `localcoder.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem localcoder:
...
Aby używać Google Vertex AI z localcoder:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania localcoder.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą localcoder, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [localcoder-helicone-session](https://github.com/H2Shami/localcoder-helicone-session), aby automatycznie rejestrować każdą konwersację localcoder jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja localcoder będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować localcoder tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować localcoder tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować localcoder tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem localcoder. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/localcoder).
:::
...
Aby korzystać z Ollama Cloud z localcoder:
...
3. Skopiuj klucz API do użycia w localcoder.
...
6. **Ważne**: Przed użyciem modeli chmurowych w localcoder musisz pobrać informacje o modelu lokalnie:
...
LocalCoder Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół localcoder. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>LocalCoder Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **LocalCoder Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API localcoder.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji localcoder.
...
5. Możesz je także dostosować za pomocą konfiguracji localcoder. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z localcoder:
...
Możesz także dostosować modele za pomocą konfiguracji localcoder. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z localcoder. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `localcoder.json` w katalogu projektu:
...
Pola `limit` pozwalają localcoder zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `localcoder auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację localcoder i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji localcoder.
