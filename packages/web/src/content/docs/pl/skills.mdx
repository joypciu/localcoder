...
Umiejętności agenta pozwalają localcoder odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
localcoder przeszukuje te lokalizacje:

- Project config: `.localcoder/skills/<name>/SKILL.md`
- Global config: `~/.config/localcoder/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu localcoder przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.localcoder/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/localcoder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.localcoder/skills/git-release/SKILL.md` w ten sposób:
...
localcoder wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `localcoder.json`:
...
**Dla agentów wbudowanych** (w `localcoder.json`):
