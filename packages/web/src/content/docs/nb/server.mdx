---
title: Tjener
description: Samhandle med LocalCoder-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `localcoder serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en LocalCoder-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `localcoder.local` |
...
Sett `LOCALCODER_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `localcoder` som standard, eller sett `LOCALCODER_SERVER_USERNAME` for å overstyre det. Dette gjelder både `localcoder serve` og `localcoder web`.
...
Når du kjører `localcoder` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk LocalCoder-serveren til å samhandle med LocalCoder programmatisk.
:::

Denne arkitekturen lar LocalCoder støtte flere klienter og lar deg samhandle med LocalCoder programmatisk.

Du kan kjøre `localcoder serve` for å starte en frittstående server. Hvis du har
LocalCoder TUI kjører, vil `localcoder serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av LocalCoder [IDE](/docs/ide) plugins.
...
localcoder-serveren viser følgende APIer.
