---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i LocalCoder.
---

import config from "../../../../config.mjs"
export const console = config.console

LocalCoder bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i LocalCoder-konfigurasjonen.
...
i `~/.local/share/localcoder/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i LocalCoder
konfig.
...
LocalCoder Zen er en liste over modeller levert av LocalCoder-teamet som har vært
testet og verifisert for å fungere godt med LocalCoder. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med LocalCoder Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `LocalCoder Zen` og gå til [localcoder.ai/auth](https://localcoder.ai/zen).
...
Det fungerer som alle andre leverandører i LocalCoder og er helt valgfritt å bruke.

---

## LocalCoder Go

LocalCoder Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av LocalCoder-teamet som har vært
testet og verifisert for å fungere godt med LocalCoder.

1. Kjør kommandoen `/connect` i TUI, velg `LocalCoder Go`, og gå til [localcoder.ai/auth](https://localcoder.ai/zen).
...
Slik bruker du Amazon Bedrock med LocalCoder:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører LocalCoder:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `localcoder.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i localcoder støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere localcoder til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at LocalCoder skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom LocalCoder-konfigurasjonen.
...
for å lagre token i LocalCoder auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
LocalCoder bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse LocalCoder
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`localcoder.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [localcoder-gitlab-auth](https://www.npmjs.com/package/localcoder-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `localcoder.json`:
...
Slik bruker du GitHub Copilot-abonnementet med LocalCoder:
...
Slik bruker du Google Vertex AI med LocalCoder:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører LocalCoder.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom LocalCoder, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [localcoder-helicone-session](https://github.com/H2Shami/localcoder-helicone-session) plugin for automatisk å logge hver LocalCoder-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver LocalCoder-samtale oppført som en separat økt.
...
Du kan konfigurere LocalCoder for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere LocalCoder for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere LocalCoder for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for LocalCoder. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/localcoder) for detaljer.
:::
...
Slik bruker du Ollama Cloud med LocalCoder:
...
3. Kopier API-nøkkelen for bruk i LocalCoder.
...
6. **Viktig**: Før du bruker skymodeller i LocalCoder, må du hente modellinformasjonen lokalt:
...
LocalCoder Zen er en liste over testede og verifiserte modeller levert av LocalCoder-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>LocalCoder Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **LocalCoder Zen**.

   ```txt
   /connect
   ```

3. Skriv inn LocalCoder API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom LocalCoder-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom LocalCoder-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med LocalCoder:
...
Du kan også tilpasse modeller gjennom LocalCoder-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med LocalCoder. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `localcoder.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar LocalCoder forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `localcoder auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk LocalCoder-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i LocalCoder-konfigurasjonen.
