...
Agentferdigheter lar LocalCoder oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
localcoder søker etter disse stedene:

- Prosjektkonfigurasjon: `.localcoder/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/localcoder/skills/<name>/SKILL.md`
...
For prosjektlokale stier går localcoder opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.localcoder/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/localcoder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.localcoder/skills/git-release/SKILL.md` slik:
...
localcoder viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `localcoder.json`:
...
**For innebygde agenter** (i `localcoder.json`):
