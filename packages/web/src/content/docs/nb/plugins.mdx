...
description: Skriv dine egne programtillegg for å utvide LocalCoder.
---

Plugins lar deg utvide LocalCoder ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til LocalCoder.
...
- `.localcoder/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/localcoder/node_modules/`.
...
1. Global konfigurasjon (`~/.config/localcoder/localcoder.json`)
2. Prosjektkonfigurasjon (`localcoder.json`)
3. Global plugin-katalog (`~/.config/localcoder/plugins/`)
4. Prosjektpluginkatalog (`.localcoder/plugins/`)
...
LocalCoder kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En LocalCoder SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide LocalCoder.
...
:::note
Hvis du bruker LocalCoder-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre LocalCoder fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til LocalCoder:
...
`tool`-hjelperen lager et tilpasset verktøy som LocalCoder kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for LocalCoder sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://localcoder.ai/docs/sdk) for detaljer.
