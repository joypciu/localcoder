...
Det er to typer agenter i LocalCoder; primære agenter og underagenter.
...
LocalCoder kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
LocalCoder kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

LocalCoder kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `localcoder.json`:
...
- Globalt: `~/.config/localcoder/agents/`
- Per prosjekt: `.localcoder/agents/`
...
Hvis ingen temperatur er spesifisert, bruker LocalCoder modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale LocalCoder-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i LocalCoder-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [LocalCoder Zen](/docs/zen), vil du bruke `localcoder/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `localcoder models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/localcoder).
:::
