...
Du kan legge til eksterne verktøy i LocalCoder ved å bruke _Model Context Protocol_ eller MCP. LocalCoder støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [LocalCoder Config](https://localcoder.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="localcoder.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/localcoder`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="localcoder.jsonc" {15}
{
  "$schema": "https://localcoder.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
LocalCoder håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil LocalCoder:
...
Hvis serveren krever autentisering, vil LocalCoder be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `localcoder mcp auth <server-name>`.
...
```json title="localcoder.json" {7-11}
{
  "$schema": "https://localcoder.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
localcoder mcp auth my-oauth-server
```
...
```bash
localcoder mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil LocalCoder lagre tokens sikkert i `~/.local/share/localcoder/mcp-auth.json`.
...
localcoder mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i LocalCoder, sammen med innebygde verktøy. Så du kan administrere dem gjennom LocalCoder-konfigurasjonen som alle andre verktøy.
...
```json title="localcoder.json" {11, 14-18}
{
  "$schema": "https://localcoder.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="localcoder.json" {4-8}
{
  "$schema": "https://localcoder.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
localcoder mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble LocalCoder til Sentry-kontoen din.
...
```json title="localcoder.json" {7-9}
{
  "$schema": "https://localcoder.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver LocalCoder-samtale oppført som en separat økt.
