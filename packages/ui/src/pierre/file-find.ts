...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("localcoder-find")
  api.delete("localcoder-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("localcoder-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("localcoder-find", new Highlight(...rest))
    return true
  }
