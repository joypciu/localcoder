...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@localcoder/acme")).toBe("@localcoder/acme")
    expect(Npm.sanitize("@localcoder/acme@1.0.0")).toBe("@localcoder/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/localcoder/acme.git"
    const expected = win ? "acme@git+https_//github.com/localcoder/acme.git" : spec
