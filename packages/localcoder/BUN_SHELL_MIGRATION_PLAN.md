...
Replace runtime Bun shell template-tag usage in `packages/localcoder/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/localcoder/src` is removed except:
