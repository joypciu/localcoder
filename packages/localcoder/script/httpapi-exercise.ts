 ...
 *
 * The script intentionally isolates `LOCALCODER_DB` before importing modules that touch
...
const preserveExerciseGlobalRoot = !!process.env.LOCALCODER_HTTPAPI_EXERCISE_GLOBAL
const exerciseGlobalRoot =
  process.env.LOCALCODER_HTTPAPI_EXERCISE_GLOBAL ??
  path.join(process.env.TMPDIR ?? "/tmp", `localcoder-httpapi-global-${process.pid}`)
process.env.XDG_DATA_HOME = path.join(exerciseGlobalRoot, "data")
process.env.XDG_CONFIG_HOME = path.join(exerciseGlobalRoot, "config")
process.env.XDG_STATE_HOME = path.join(exerciseGlobalRoot, "state")
process.env.XDG_CACHE_HOME = path.join(exerciseGlobalRoot, "cache")
process.env.LOCALCODER_DISABLE_SHARE = "true"
const exerciseConfigDirectory = path.join(exerciseGlobalRoot, "config", "localcoder")
const exerciseDataDirectory = path.join(exerciseGlobalRoot, "data", "localcoder")

const preserveExerciseDatabase = !!process.env.LOCALCODER_HTTPAPI_EXERCISE_DB
const exerciseDatabasePath =
  process.env.LOCALCODER_HTTPAPI_EXERCISE_DB ??
  path.join(process.env.TMPDIR ?? "/tmp", `localcoder-httpapi-exercise-${process.pid}.db`)
process.env.LOCALCODER_DB = exerciseDatabasePath
Flag.LOCALCODER_DB = exerciseDatabasePath

void (await import("@localcoder-ai/core/util/log")).init({ print: false })
...
const original = {
  LOCALCODER_EXPERIMENTAL_HTTPAPI: Flag.LOCALCODER_EXPERIMENTAL_HTTPAPI,
...
const scenarios: Scenario[] = [
  ...
  http
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "localcoder.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "localcoder.jsonc")).text(),
          )
  ...
  http.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-localcoder-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-localcoder-directory")
  }),
  ...
  http
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "localcoder", model: "test" })}`,
      headers: ctx.headers(),
    }))
    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
        ...
        const run = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
          effect.pipe(Effect.provideService(modules.InstanceRef, instance), Effect.provide(modules.AppLayer), Effect.orDie)
        const directory = () => {
        ...
        const base: ScenarioContext = {
          ...
          headers: (extra) => ({
            ...(context.dir?.path ? { "x-localcoder-directory": context.dir.path } : {}),
            ...extra,
          }),
          file: (name, content) =>
            Effect.promise(() => {
          ...
          message: (sessionID, input) =>
            Effect.gen(function* () {
              const info: MessageV2.User = {
                ...
                agent: "build",
                model: {
                  providerID: ProviderID.localcoder,
...
function app(modules: Runtime, backend: Backend) {
  Flag.LOCALCODER_EXPERIMENTAL_HTTPAPI = backend === "effect"
    ...
    modules.ExperimentalHttpApiServer.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ LOCALCODER_SERVER_PASSWORD: undefined, LOCALCODER_SERVER_USERNAME: undefined }),
        ),
      ),
    ),
    { disableLogger: true },
...
const resetState = Effect.promise(async () => {
  ...
  Flag.LOCALCODER_EXPERIMENTAL_HTTPAPI = original.LOCALCODER_EXPERIMENTAL_HTTPAPI
