...
const originalHttpApi = Flag.LOCALCODER_EXPERIMENTAL_HTTPAPI
const it = testEffect(
...
function request(path: string, directory: string, init: RequestInit = {}, httpApi = true) {
  return Effect.promise(() => {
    Flag.LOCALCODER_EXPERIMENTAL_HTTPAPI = httpApi
    const headers = new Headers(init.headers)
    headers.set("x-localcoder-directory", directory)
...
afterEach(async () => {
  ...
  Flag.LOCALCODER_EXPERIMENTAL_HTTPAPI = originalHttpApi
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-localcoder-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://localcoder.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-localcoder-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-localcoder-workspace")
      } finally {
