...
describe("session HttpApi", () => {
  it.live(
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false } }, (tmp) =>
      Effect.gen(function* () {
        const headers = { "x-localcoder-directory": tmp.path }
  ...
  it.live(
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false, share: "disabled" } }, (tmp) =>
      Effect.gen(function* () {
        const headers = { "x-localcoder-directory": tmp.path, "content-type": "application/json" }
  ...
  it.live(
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false } }, (tmp) =>
      Effect.gen(function* () {
        const currentDir = path.join(tmp.path, "packages", "localcoder", "src")
