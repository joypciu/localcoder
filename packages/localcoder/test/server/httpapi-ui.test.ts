...
const original = {
  ...
  LOCALCODER_DISABLE_EMBEDDED_WEB_UI: Flag.LOCALCODER_DISABLE_EMBEDDED_WEB_UI,
  LOCALCODER_SERVER_PASSWORD: Flag.LOCALCODER_SERVER_PASSWORD,
  LOCALCODER_SERVER_USERNAME: Flag.LOCALCODER_SERVER_USERNAME,
  envPassword: process.env.LOCALCODER_SERVER_PASSWORD,
  envUsername: process.env.LOCALCODER_SERVER_USERNAME,
}

afterEach(() => {
  Flag.LOCALCODER_EXPERIMENTAL_HTTPAPI = original.LOCALCODER_EXPERIMENTAL_HTTPAPI
  Flag.LOCALCODER_DISABLE_EMBEDDED_WEB_UI = original.LOCALCODER_DISABLE_EMBEDDED_WEB_UI
  ...
  restoreEnv("LOCALCODER_SERVER_PASSWORD", original.envPassword)
  restoreEnv("LOCALCODER_SERVER_USERNAME", original.envUsername)
})
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            LOCALCODER_SERVER_PASSWORD: input?.password,
            LOCALCODER_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  test("serves the web UI through the experimental backend", async () => {
    ...
    Flag.LOCALCODER_DISABLE_EMBEDDED_WEB_UI = true
    let proxiedUrl: string | undefined

    const response = await uiApp({
      client: httpClient(
        new Response("<html>localcoder</html>", { headers: { "content-type": "text/html" } }),
        (request) => {
    ...
    expect(await response.text()).toBe("<html>localcoder</html>")
    expect(proxiedUrl).toBe("https://app.localcoder.ai/")
  })
  ...
  test("strips upstream transfer encoding headers from proxied assets", async () => {
    ...
    expect(response.status).toBe(200)
    expect(proxiedUrl).toBe("https://app.localcoder.ai/assets/app.js")
            ...
            Layer.succeed(
              ...
              HttpClient.make((request) =>
                Effect.succeed(
                  HttpClientResponse.fromWeb(
                    request,
                    new Response("<html>localcoder</html>", {
                      headers: {
  ...
  test("requires server password for the web UI", async () => {
    ...
    const response = await uiApp({ password: "secret", username: "localcoder" }).request("/")
  ...
  test("accepts auth token for the web UI", async () => {
    ...
    const response = await uiApp({
      password: "secret",
      username: "localcoder",
      client: httpClient(new Response("<html>localcoder</html>", { headers: { "content-type": "text/html" } })),
    }).request(`/?auth_token=${btoa("localcoder:secret")}`)
  ...
  test("accepts basic auth for the web UI", async () => {
    ...
    const response = await uiApp({ password: "secret", username: "localcoder" }).request("/", {
      headers: { authorization: `Basic ${btoa("localcoder:secret")}` },
    })
  ...
  test("allows web UI preflight without auth", async () => {
    ...
    const response = await app({ password: "secret", username: "localcoder" }).request("/", {
      method: "OPTIONS",
      headers: {
