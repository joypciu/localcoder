...
const originalWorkspaces = Flag.LOCALCODER_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.LOCALCODER_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session.list", () => {
  test("does not filter by directory when directory is omitted", async () => {
    ...
    await mkdir(path.join(tmp.path, "packages", "localcoder"), { recursive: true })
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const current = await WithInstance.provide({
          directory: path.join(tmp.path, "packages", "localcoder"),
  ...
  test("filters by directory when directory is provided", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const ids = (await svc.list({ directory: path.join(tmp.path, "packages", "localcoder") })).map((s) => s.id)
  ...
  test("filters by path and ignores directory when path is provided", async () => {
    Flag.LOCALCODER_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "localcoder", "src", "deep"), { recursive: true })
    await mkdir(path.join(tmp.path, "packages", "app"), { recursive: true })
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const current = await WithInstance.provide({
          directory: path.join(tmp.path, "packages", "localcoder", "src"),
          fn: async () => svc.create({ title: "current" }),
        })
        const deeper = await WithInstance.provide({
          directory: path.join(tmp.path, "packages", "localcoder", "src", "deep"),
  ...
  test("falls back to directory when filtering legacy sessions without path", async () => {
    Flag.LOCALCODER_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "localcoder", "src"), { recursive: true })
    await mkdir(path.join(tmp.path, "packages", "app"), { recursive: true })
