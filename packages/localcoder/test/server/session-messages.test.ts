...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = process.env.LOCALCODER_EXPERIMENTAL_DISABLE_FILEWATCHER
  process.env.LOCALCODER_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) delete process.env.LOCALCODER_EXPERIMENTAL_DISABLE_FILEWATCHER
    else process.env.LOCALCODER_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}
