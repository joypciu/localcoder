...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under localcoder.", () => {
    expect(paramToAttributeKey("name")).toBe("localcoder.name")
    expect(paramToAttributeKey("slug")).toBe("localcoder.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["localcoder.sessionID"]).toBeUndefined()
    expect(attrs["localcoder.messageID"]).toBeUndefined()
    expect(attrs["localcoder.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under localcoder. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["localcoder.name"]).toBe("exa")
