...
import { createlocalcoderClient } from "@localcoder-ai/sdk/v2"
...
type Sdk = ReturnType<typeof createlocalcoderClient>
...
function app(backend: Backend, input?: { password?: string; username?: string }) {
  Flag.LOCALCODER_EXPERIMENTAL_HTTPAPI = backend === "httpapi"
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("localcoder", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("localcoder", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
