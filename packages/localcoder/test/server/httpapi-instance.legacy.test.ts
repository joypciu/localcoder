...
const original = Flag.LOCALCODER_EXPERIMENTAL_HTTPAPI
...
afterEach(async () => {
  Flag.LOCALCODER_EXPERIMENTAL_HTTPAPI = original
...
describe("instance HttpApi", () => {
  test("serves catalog read endpoints through Hono bridge", async () => {
    ...
    const [commands, agents, skills, lsp, formatter] = await Promise.all([
      app().request(InstancePaths.command, { headers: { "x-localcoder-directory": tmp.path } }),
      app().request(InstancePaths.agent, { headers: { "x-localcoder-directory": tmp.path } }),
      app().request(InstancePaths.skill, { headers: { "x-localcoder-directory": tmp.path } }),
      app().request(InstancePaths.lsp, { headers: { "x-localcoder-directory": tmp.path } }),
      app().request(InstancePaths.formatter, { headers: { "x-localcoder-directory": tmp.path } }),
    ])
  ...
  test("serves project git init through Hono bridge", async () => {
    ...
    const current = await app().request("/project/current", { headers: { "x-localcoder-directory": tmp.path } })
  ...
  test("serves project update through Hono bridge", async () => {
    ...
    const list = await app().request("/project", { headers: { "x-localcoder-directory": tmp.path } })
