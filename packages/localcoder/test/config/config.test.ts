...
import { EffectFlock } from "@localcoder-ai/core/util/effect-flock"
...
const managedConfigDir = process.env.LOCALCODER_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "localcoder.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "localcoder.json") {
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "localcoder.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "localcoder.jsonc"),
        JSON.stringify({
          $schema: "https://localcoder.ai/config.json",
          shell: "bash",
          model: "test/model",
  ...
  try {
    ...
    const file = path.join(tmp.path, "localcoder.jsonc")
...
test("ignores legacy tui keys in localcoder config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://localcoder.ai/config.json",
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "localcoder.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["LOCALCODER_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { localcoder: { options: { apiKey: "{env:LOCALCODER_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["localcoder"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["LOCALCODER_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["LOCALCODER_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "localcoder.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .localcoder directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const localcoderDir = path.join(dir, ".localcoder")
      await fs.mkdir(localcoderDir, { recursive: true })
      const agentDir = path.join(localcoderDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".localcoder", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .localcoder/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const localcoderDir = path.join(dir, ".localcoder")
      await fs.mkdir(localcoderDir, { recursive: true })

      const agentsDir = path.join(localcoderDir, "agents")
...
test("loads commands from .localcoder/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const localcoderDir = path.join(dir, ".localcoder")
      await fs.mkdir(localcoderDir, { recursive: true })

      const commandDir = path.join(localcoderDir, "command")
...
test("loads commands from .localcoder/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const localcoderDir = path.join(dir, ".localcoder")
      await fs.mkdir(localcoderDir, { recursive: true })

      const commandsDir = path.join(localcoderDir, "commands")
...
test("does not try to install dependencies in read-only LOCALCODER_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.LOCALCODER_CONFIG_DIR
  process.env.LOCALCODER_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.LOCALCODER_CONFIG_DIR
    else process.env.LOCALCODER_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable LOCALCODER_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "localcoder.json"),
        JSON.stringify({ $schema: "https://localcoder.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .localcoder config
      const projectDir = path.join(dir, "project")
      const localcoderDir = path.join(projectDir, ".localcoder")
      await fs.mkdir(localcoderDir, { recursive: true })
      ...
      // Local .localcoder config with different plugins
      await Filesystem.write(
        path.join(localcoderDir, "localcoder.json"),
        JSON.stringify({
          $schema: "https://localcoder.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .localcoder config with some overlapping plugins
      await Filesystem.write(
        path.join(localcoderDir, "localcoder.json"),
        JSON.stringify({
          $schema: "https://localcoder.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".localcoder")
      ...
      await Filesystem.write(
        path.join(local, "localcoder.json"),
        JSON.stringify({
          $schema: "https://localcoder.ai/config.json",
...
// Note: preload.ts sets LOCALCODER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .localcoder config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .localcoder directory config enables it
      const localcoderDir = path.join(dir, ".localcoder")
      await fs.mkdir(localcoderDir, { recursive: true })
      await Filesystem.write(
        path.join(localcoderDir, "localcoder.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/localcoder")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/localcoder")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "localcoder.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-localcoder@2.4.3", file)).toBe("oh-my-localcoder@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-localcoder@2.4.3", "file:///project/.localcoder/plugin/oh-my-localcoder.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.localcoder/plugin/demo.ts", "file:///project/.localcoder/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.localcoder/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const localcoderDir = path.join(projectDir, ".localcoder")
        const pluginDir = path.join(localcoderDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("LOCALCODER_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"]
    process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .localcoder/ directories when flag is set", async () => {
    const originalEnv = process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"]
    process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .localcoder directory with a command
          const localcoderDir = path.join(dir, ".localcoder", "command")
          await fs.mkdir(localcoderDir, { recursive: true })
          await Filesystem.write(path.join(localcoderDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await WithInstance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .localcoder should NOT be in directories list
          const hasProjectlocalcoder = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectlocalcoder).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LOCALCODER_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["LOCALCODER_CONFIG_DIR"]
      process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["LOCALCODER_CONFIG_DIR"]
      } else {
        process.env["LOCALCODER_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("LOCALCODER_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LOCALCODER_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["LOCALCODER_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["LOCALCODER_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from LOCALCODER_CONFIG_DIR, not project
...
describe("LOCALCODER_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in LOCALCODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["LOCALCODER_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["LOCALCODER_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://localcoder.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["LOCALCODER_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["LOCALCODER_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in LOCALCODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["LOCALCODER_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "LocalCoder Managed",
          PayloadIdentifier: "ai.localcoder.managed.test",
          PayloadType: "ai.localcoder.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://localcoder.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://localcoder.ai/config.json")
})
