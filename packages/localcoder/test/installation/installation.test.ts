...
describe("installation", () => {
  describe("latest", () => {
    ...
    test("reads npm versions via registry", async () => {
      ...
      expect(calls).toContain(`https://registry.npmjs.org/localcoder-ai/${InstallationChannel}`)
    })
    ...
    test("reads brew formulae API versions", async () => {
      const layer = testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("anomalyco/tap/localcoder")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("localcoder")) return "localcoder"
          return ""
        },
      )
    ...
    test("reads brew tap info JSON via CLI", async () => {
      ...
      const layer = testLayer(
        ...
        (cmd, args) => {
          if (cmd === "brew" && args.includes("anomalyco/tap/localcoder") && args.includes("--formula")) return "localcoder"
