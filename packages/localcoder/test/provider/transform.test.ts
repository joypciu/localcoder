...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const localcoderModel = {
      ...openaiModel,
      providerID: "localcoder",
      api: {
        id: "localcoder-test",
        url: "https://api.localcoder.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, localcoderModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.localcoder?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.localcoder?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          localcoder: { itemId: "msg_localcoder" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              localcoder: { itemId: "msg_localcoder_part" },
    ...
    expect(result[0].providerOptions?.localcoder?.itemId).toBe("msg_localcoder")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.localcoder?.itemId).toBe("msg_localcoder_part")
