...
const originalWorkspacesFlag = Flag.LOCALCODER_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  LOCALCODER_AUTH_CONTENT: process.env.LOCALCODER_AUTH_CONTENT,
...
afterEach(async () => {
  ...
  Flag.LOCALCODER_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
describe("workspace-old CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      process.env.LOCALCODER_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
      process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
      process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=localcoder-test"
      ...
      expect(JSON.parse(recorded.calls.create[0].env.LOCALCODER_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.LOCALCODER_WORKSPACE_ID).toBe(workspaceID)
      expect(recorded.calls.create[0].env.LOCALCODER_EXPERIMENTAL_WORKSPACES).toBe("true")
      ...
      expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=localcoder-test")
...
describe("workspace-old sync state", () => {
  test("startWorkspaceSyncing is disabled by the experimental workspace flag", async () => {
    await withInstance(async (dir) => {
      Flag.LOCALCODER_EXPERIMENTAL_WORKSPACES = false
