...
- `config?: Partial<Config.Info>` - Write an `localcoder.json` config file
...
- Directories are created in the system temp folder with prefix `localcoder-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("localcoder-test-")
  }),
)
```
