...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.LOCALCODER_TEST ?? "")'), {
      env: {
        LOCALCODER_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "LOCALCODER_TEST_SHELL"], {
      shell: true,
      env: {
        LOCALCODER_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("LOCALCODER_TEST_SHELL=ok")
  })
