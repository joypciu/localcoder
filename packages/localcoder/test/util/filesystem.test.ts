...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "localcoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "localcoder.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "localcoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "localcoder.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["localcoder.json", "localcoder.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "localcoder.json"),
        path.join(tmp.path, "localcoder.jsonc"),
        path.join(project, "localcoder.json"),
        path.join(project, "localcoder.jsonc"),
      ])
    })
  })
