...
- Author package entrypoint is `@localcoder-ai/plugin/tui`.
...
```json
{
  "$schema": "https://localcoder.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/localcoder-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- The value is a semver range checked against the running LocalCoder version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.localcoder` is absent, no check is performed (backward compatible).
...
- `localcoder plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `localcoder plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.localcoder` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.localcoder`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.localcoder`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.localcoder` dir or `LOCALCODER_CONFIG_DIR`, LocalCoder installs `@localcoder-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@localcoder-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.localcoder/themes` area near the plugin config source.
...
- `--pure` / `LOCALCODER_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.localcoder/plugins/tui-smoke.tsx`
- Local vim plugin: `.localcoder/plugins/tui-vim.tsx`
- Local smoke config: `.localcoder/tui.json`
- Local smoke theme: `.localcoder/plugins/smoke-theme.json`
