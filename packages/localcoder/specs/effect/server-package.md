...
Practical reference for extracting a future `packages/server` from the current `packages/localcoder` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all localcoder services, Effect-first source of truth
- `packages/server` - localcoder server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full localcoder server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/localcoder`.
...
- those slices are mounted into the Hono server behind `LOCALCODER_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `localcoder` package split is earlier in the migration.
...
- the current server host still lives in `packages/localcoder`
...
- if `packages/server` imports services from `packages/localcoder` while `packages/localcoder` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/localcoder` as the temporary Hono host
...
- `packages/server` must not import from `packages/localcoder`

Allowed in phase 1:

- `packages/localcoder` imports `packages/server`
...
- `packages/localcoder` shrinks or disappears as package responsibilities are fully split
...
- `packages/localcoder` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/localcoder`
...
Keep in `packages/localcoder` for now:
...
- host mounting from `packages/localcoder`
...
- no imports from `packages/localcoder` inside `packages/server`
...
- `packages/server` must still not import from `packages/localcoder`
...
### PR 4. Mount `packages/server` question from `packages/localcoder`

Scope:

- replace local experimental question route wiring in `packages/localcoder`
...
- mount it in parallel from `packages/localcoder`
...
- move adapter selection and server startup out of `packages/localcoder`

Rules:

- do not start this while `packages/server` still depends on `packages/localcoder`
...
- host logic no longer lives in `packages/localcoder`
...
3. the route is mounted from the current host in `packages/localcoder`
...
- `packages/server` imports services or runtime from `packages/localcoder`
- `packages/localcoder` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/localcoder` imports
...
- [ ] mount `question` from `packages/localcoder`
...
2. keep `packages/localcoder` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/localcoder`, stop and restructure the boundary first.
