...
import { Module } from "@localcoder-ai/core/util/module"
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.LOCALCODER_DISABLE_LSP_DOWNLOAD) return
...
export const Ty: Info = {
  ...
  async spawn(root) {
    if (!Flag.LOCALCODER_EXPERIMENTAL_LSP_TY) {
      return undefined
    }
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "localcoder-jdtls-data"))
    return {
      process: spawn(
        java,
        [
