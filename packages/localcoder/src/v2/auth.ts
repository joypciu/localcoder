...
import { Identifier } from "@localcoder-ai/core/util/identifier"
import { NonNegativeInt, withStatics } from "@/util/schema"
import { Global } from "@localcoder-ai/core/global"
import { AppFileSystem } from "@localcoder-ai/core/filesystem"

export const OAUTH_DUMMY_KEY = "localcoder-oauth-dummy-key"
...
export class Service extends Context.Service<Service, Interface>()("@localcoder/v2/Auth") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const load: () => Effect.Effect<Writable, AuthError> = Effect.fnUntraced(function* () {
      if (process.env.LOCALCODER_AUTH_CONTENT) {
        try {
          return JSON.parse(process.env.LOCALCODER_AUTH_CONTENT)
        } catch {}
      }
