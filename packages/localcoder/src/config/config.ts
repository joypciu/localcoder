...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in localcoder config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for localcoder serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://localcoder.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://localcoder.ai/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@localcoder/Config") {}

function globalConfigFile() {
  const candidates = ["localcoder.jsonc", "localcoder.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://localcoder.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "localcoder.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "localcoder.jsonc")))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://localcoder.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "LOCALCODER_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/localcoder` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/localcoder`))
            if (!response.ok) {
              throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
            }
            const wellknown = (yield* Effect.promise(() => response.json())) as { config?: Record<string, unknown> }
            const remoteConfig = wellknown.config ?? {}
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://localcoder.ai/config.json"
            const source = `${url}/.well-known/localcoder`
        ...
        if (Flag.LOCALCODER_CONFIG) {
          yield* merge(Flag.LOCALCODER_CONFIG, yield* loadFile(Flag.LOCALCODER_CONFIG))
          log.debug("loaded custom config", { path: Flag.LOCALCODER_CONFIG })
        }

        if (!Flag.LOCALCODER_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("localcoder", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.LOCALCODER_CONFIG_DIR) {
          log.debug("loading config from LOCALCODER_CONFIG_DIR", { path: Flag.LOCALCODER_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".localcoder") || dir === Flag.LOCALCODER_CONFIG_DIR) {
            for (const file of ["localcoder.json", "localcoder.jsonc"]) {
          ...
          // Auto-discovered plugins under `.localcoder/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.LOCALCODER_CONFIG_CONTENT) {
          const source = "LOCALCODER_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.LOCALCODER_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from LOCALCODER_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["LOCALCODER_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("LOCALCODER_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.LOCALCODER_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.LOCALCODER_PERMISSION))
        }
        ...
        if (Flag.LOCALCODER_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.LOCALCODER_DISABLE_PRUNE) {
