...
const MANAGED_PLIST_DOMAIN = "ai.localcoder.managed"

// Keys injected by macOS/MDM into the managed plist that are not LocalCoder config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/localcoder"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "localcoder")
    default:
      return "/etc/localcoder"
  }
}

export function managedConfigDir() {
  return process.env.LOCALCODER_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
