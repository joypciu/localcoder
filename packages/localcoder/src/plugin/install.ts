...
export type PatchDeps = {
  ...
  files: (dir: string, name: "localcoder" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".localcoder")
}

function patchName(kind: Kind): "localcoder" | "tui" {
  if (kind === "server") return "localcoder"
  return "tui"
}
