...
export const DEPRECATED_PLUGIN_PACKAGES = ["localcoder-openai-codex-auth", "localcoder-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, localcoderVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(localcoderVersion) || semver.major(localcoderVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.localcoder
  if (typeof range !== "string") return
  if (!semver.satisfies(localcoderVersion, range)) {
    throw new Error(`Plugin requires localcoder ${range} but running ${localcoderVersion}`)
  }
}
