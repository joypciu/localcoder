...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall localcoder and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall LocalCoder")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g localcoder-ai",
      pnpm: "pnpm uninstall -g localcoder-ai",
      bun: "bun remove -g localcoder-ai",
      yarn: "yarn global remove localcoder-ai",
      brew: "brew uninstall localcoder",
      choco: "choco uninstall localcoder",
      scoop: "scoop uninstall localcoder",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "localcoder", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using LocalCoder!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# localcoder") || content.includes(".localcoder/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".localcoder/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".localcoder/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".localcoder"))
    ) {
      continue
    }
