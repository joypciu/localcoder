import { BoxRenderable, RGBA, TextareaRenderable, MouseEvent, MouseButton, PasteEvent, decodePasteBytes } from "@opentui/core"
...
export function Prompt(props: PromptProps) {
  ...
  command.register(() => {
    return [
      ...
      },
      {
        ...
        value: "workspace.set",
        category: "Session",
        enabled: Flag.LOCALCODER_EXPERIMENTAL_WORKSPACES,
        slash: {
          name: "warp",
        },
        onSelect: (dialog) => {
  ...
  function wordRangeAt(text: string, offset: number) {
    ...
    return {
      start: left,
      end: right === -1 ? text.length : index + right,
    }
  }
          ...
          >
            <textarea
              ...
              onKeyDown={async (e) => {
                ...
                if (keybind.match("input_select_all", e)) {
                  e.preventDefault()
                  input.selectAll()
                  return
                }
                // Duplicate current line
                if (keybind.match("input_duplicate_line", e)) {
                  e.preventDefault()
                  const text = input.plainText
                  const pos = input.cursorOffset
                  ...
                  input.cursorOffset = pos + lines[lineIdx].length + 1
                  return
                }
                // Move current line up
                if (keybind.match("input_move_line_up", e)) {
                  e.preventDefault()
                  const text = input.plainText
                  const pos = input.cursorOffset
                  const lines = text.split("\n")
                  ...
                  if (lineIdx > 0) {
                    const moved = lines.splice(lineIdx, 1)[0]
                    lines.splice(lineIdx - 1, 0, moved)
                    input.setText(lines.join("\n"))
                    input.cursorOffset = Math.max(0, pos - (lines[lineIdx].length + 1))
                  }
                  return
                }
                // Move current line down
                if (keybind.match("input_move_line_down", e)) {
                  e.preventDefault()
                  const text = input.plainText
                  const pos = input.cursorOffset
                  const lines = text.split("\n")
                  ...
                  if (lineIdx < lines.length - 1) {
                    ...
                    lines.splice(lineIdx + 1, 0, moved)
                    input.setText(lines.join("\n"))
                    input.cursorOffset = pos + lines[lineIdx + 1].length + 1
                  }
                  return
                }
                // Toggle case of selection
                if (keybind.match("input_toggle_case", e)) {
                  e.preventDefault()
                  const text = input.plainText
                  const [start, end] = input.hasSelection()
                    ? [input.visualCursor.offset, input.visualCursor.offset + (input.getSelectedText()?.length ?? 0)]
                    : [0, text.length]
                  const toggled = text
                    .slice(start, end)
                    .split("")
                    ...
                    .join("")
                  input.setText(text.slice(0, start) + toggled + text.slice(end))
