...
import type { TuiThemeCurrent } from "@localcoder-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "localcoder")
        draft.active = typeof active === "string" ? active : "localcoder"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.localcoder, store.mode)
    })
