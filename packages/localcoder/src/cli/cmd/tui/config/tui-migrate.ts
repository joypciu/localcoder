...
const TUI_SCHEMA_URL = "https://localcoder.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from localcoder.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const localcoder = await localcoderFiles(input)
  for (const file of localcoder) {
...
async function localcoderFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "localcoder"),
    ...(await Filesystem.findUp(["localcoder.json", "localcoder.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "localcoder"))
  }
  if (Flag.LOCALCODER_CONFIG) files.push(Flag.LOCALCODER_CONFIG)
