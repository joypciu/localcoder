...
import { createlocalcoderClient, type localcoderClient, type ToolPart } from "@localcoder-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run localcoder with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running localcoder server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to LOCALCODER_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to LOCALCODER_SERVER_USERNAME or 'localcoder')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: localcoderClient) {
      ...
      async function share(sdk: localcoderClient, sessionID: string) {
        const cfg = await sdk.config.get()
        if (!cfg.data) return
        if (cfg.data.share !== "auto" && !Flag.LOCALCODER_AUTO_SHARE && !args.share) return
      ...
      async function execute(sdk: localcoderClient) {
      ...
      if (args.attach) {
        ...
        const sdk = createlocalcoderClient({ baseUrl: args.attach, directory, headers })
      ...
      const sdk = createlocalcoderClient({ baseUrl: "http://localcoder.internal", fetch: fetchFn })
      await execute(sdk)
    })
  }),
})
