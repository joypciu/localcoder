...
export const OUTPUT_TOKEN_MAX = Flag.LOCALCODER_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "localcoder" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.providerID === "llamacpp" && input.model.api.npm === "@ai-sdk/openai-compatible") {
    ...
    if (id.includes("qwen") || id.includes("qwopus")) {
      // some llama.cpp builds returns empty content when enable_thinking=false (text in reasoning_content only).
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    ...
    if (input.model.providerID.startsWith("localcoder")) {
      result["promptCacheKey"] = input.sessionID
