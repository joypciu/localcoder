...
import { Flock } from "@localcoder-ai/core/util/flock"
...
export class Service extends Context.Service<Service, Interface>()("@localcoder/ModelsDev") {}
...
export const layer: Layer.Layer<Service, never, AppFileSystem.Service | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.LOCALCODER_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.LOCALCODER_MODELS_PATH ?? filepath).pipe(
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.LOCALCODER_DISABLE_MODELS_FETCH) return {}
      // Flock is cross-process: concurrent localcoder CLIs can race on this cache file.
    ...
    if (!Flag.LOCALCODER_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
