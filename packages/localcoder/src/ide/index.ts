...
export function alreadyInstalled() {
  return process.env["LOCALCODER_CALLER"] === "vscode" || process.env["LOCALCODER_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.localcoder"], {
    nothrow: true,
  })
