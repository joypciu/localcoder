...
import * as EffectLogger from "@localcoder-ai/core/effect/logger"
...
export interface Interface {
  ...
  readonly prompt: (input: PromptInput) => Effect.Effect<MessageV2.WithParts, any, any>
  readonly loop: (input: LoopInput) => Effect.Effect<MessageV2.WithParts, any, any>
  readonly shell: (input: ShellInput) => Effect.Effect<MessageV2.WithParts, any, any>
  readonly command: (input: CommandInput) => Effect.Effect<MessageV2.WithParts, any, any>
  readonly resolvePromptParts: (template: string) => Effect.Effect<PromptInput["parts"]>
}

export class Service extends Context.Service<Service, Interface>()("@localcoder/SessionPrompt") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const insertReminders = Effect.fn("SessionPrompt.insertReminders")(function* (input: {
      ...
      if (!Flag.LOCALCODER_EXPERIMENTAL_PLAN_MODE) {
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
      ...
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          const { msg, part, cwd } = yield* Effect.gen(function* () {
            ...
            SessionEventEmit.emit(SessionEvent.Shell.Started.Sync, {
              sessionID: input.sessionID,
          ...
          const finish = Effect.uninterruptible(
            Effect.gen(function* () {
              ...
              SessionEventEmit.emit(SessionEvent.Shell.Ended.Sync, {
                sessionID: input.sessionID,
    ...
    const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
      ...
      if (current?.agent !== info.agent) {
        SessionEventEmit.emit(SessionEvent.AgentSwitched.Sync, {
          sessionID: input.sessionID,
      ...
      if (
        ...
        SessionEventEmit.emit(SessionEvent.ModelSwitched.Sync, {
          sessionID: input.sessionID,
          timestamp: DateTime.makeUnsafe(info.time.created),
          model: {
      ...
      SessionEventEmit.emit(SessionEvent.Prompted.Sync, {
        sessionID: input.sessionID,
        timestamp: DateTime.makeUnsafe(info.time.created),
        prompt: {
      ...
      for (const text of nextPrompt.synthetic) {
        // TODO(v2): Temporary dual-write while migrating session messages to v2 events.
        SessionEventEmit.emit(SessionEvent.Synthetic.Sync, {
          sessionID: input.sessionID,
          timestamp: DateTime.makeUnsafe(info.time.created),
          text,
        })
    ...
    const runLoop: (sessionID: SessionID) => Effect.Effect<MessageV2.WithParts, any, any> = Effect.fn("SessionPrompt.run")(
