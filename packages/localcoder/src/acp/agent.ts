...
import type { AssistantMessage, Event, localcoderClient, SessionMessageResponse, ToolPart } from "@localcoder-ai/sdk/v2"
...
export function init({ sdk: _sdk }: { sdk: localcoderClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `localcoder auth login` in the terminal",
      name: "Login with localcoder",
      id: "localcoder-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // LocalCoder stores files internally as { type: "file", url, filename, mime }.
