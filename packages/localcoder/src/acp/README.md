...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for localcoder.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal localcoder sessions
...
localcoder acp --cwd /path/to/project
```
...
```bash
LOCALCODER_ENABLE_QUESTION_TOOL=1 localcoder acp
```
...
- **Terminal Integration**: Full terminal support via localcoder's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | localcoder acp
```
...
### Mapping to LocalCoder

ACP sessions map cleanly to localcoder's internal session model:
