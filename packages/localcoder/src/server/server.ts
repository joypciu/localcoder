...
import { maybeAutoStartLlamaCpp } from "@/llamacpp/autostart"
...
function createHono(opts: CorsOptions, selection: ServerBackend.Selection = ServerBackend.force(select(), "hono")) {
  ...
  if (Flag.LOCALCODER_WORKSPACE_ID) {
    return {
      app: app
        .use(InstanceMiddleware(Flag.LOCALCODER_WORKSPACE_ID ? WorkspaceID.make(Flag.LOCALCODER_WORKSPACE_ID) : undefined))
        .use(FenceMiddleware)
 ...
 * `PublicApi` is `LocalCoderHttpApi` annotated with the `matchLegacyOpenApi`
...
export async function listen(opts: ListenOptions): Promise<Listener> {
  ...
  void maybeAutoStartLlamaCpp().catch((error) => {
    log.warn("llamacpp autostart error", { error: error instanceof Error ? error.message : String(error) })
  })
...
async function listenHttpApi(opts: ListenOptions, selection: ServerBackend.Selection): Promise<Listener> {
  log.info("server backend selected", {
    ...
    "localcoder.server.runtime": HttpApiServer.name,
  })
