...
import { LocalCoderHttpApi } from "./api"
...
const LegacyComponentDescriptions = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for localcoder serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw = OpenApi.fromApi(LocalCoderHttpApi) as unknown as OpenApiSpec
...
export const PublicApi = LocalCoderHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "localcoder",
    version: "1.0.0",
