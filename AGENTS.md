...
- Tests cannot run from repo root (guard: `do-not-run-tests-from-root`); run from package dirs like `packages/localcoder`.
...
This repo lives under the `local_ai_agents` workspace and integrates with Plex (mcp_gguf_agent) and jclaw. A standalone real-server smoke test is at `test_localcoder_standalone.py`.\n\nEnd users can launch LocalCoder from the shared workspace launchers:\n- GUI: `python launcher_gui.py`\n- CLI: `python launcher_cli.py tool localcoder-desktop`
